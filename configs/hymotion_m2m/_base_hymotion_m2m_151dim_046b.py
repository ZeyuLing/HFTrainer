# HyMotion M2M 0.46B base config — 151-dim with end-effector positions + foot contact
#
...
#
# Foot contact binary indicators (SMPL-22 indices, velocity-based):
#   - L_Wrist (20): dim [147]
#   - R_Wrist (21): dim [148]
#   - L_Foot (10): dim [149]
#   - R_Foot (11): dim [150]
...
#   input_dim = 151 + 3*151 = 604 (motion + VACE context)
#   output_dim = 151

_base_ = '../_base_/default_runtime.py'

work_dir = 'work_dirs/hymotion_m2m_completion_151dim_046b'
...
#   time_factor=1000.0
#
# For M2M with 151-dim: input_dim = 151 + 3*151 = 604
# output_dim = 151
_motion_dim = 151  # smpl_22 with rot6d + abs transl + end-effector pos + foot contact: 3 + 132 + 12 + 4 = 151
...
model = dict(
    ...
    motion_transformer=dict(
        ...
        trainable=True,
        input_dim=_motion_dim + 3 * _motion_dim,  # 604 (motion + VACE context)
        feat_dim=1024,
        output_dim=_motion_dim,                    # 151
        ctxt_input_dim=4096,
        vtxt_input_dim=768,
        num_layers=18,
        num_heads=16,
    ...
    text_encoder=dict(),
    mean_std_dir='data/hymotion_m2m_data/_stats_151dim',  # 151-dim Mean/Std
    motion_type='smpl_22',
    pred_type='velocity',
    ...
    losses_cfg=dict(
        ...
        foot_contact_weight=3.0,  # Foot contact BCE loss weight (γ_contact from roadmap)
        foot_contact_warmup_steps=5000,  # Warmup scheduling for contact loss (half of FK warmup)
    ),
