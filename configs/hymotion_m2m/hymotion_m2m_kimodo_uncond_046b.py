# HyMotion M2M v2 — PHASE 0 E3: KIMODO Root + Unconditioned
#
# **Experiment E3** from next-gen proposal: SMPL motion converted to KIMODO
# Root representation (with online ADMM smoothing) and unconditional generation.
#
...
#
# E3 introduces KIMODO Root which should improve:
#   - Root trajectory consistency (ADMM smoothing bounds pelvis drift)
#   - Position-to-root relative coordinates for better structural learning
#   - FK consistency supervision via fk_consistency_weight=1500.0
#
# Prerequisite: KIMODO Root mean/std must be computed and placed at
#   data/hymotion_m2m_data/_stats_198dim_kimodo_root/
#
# Launch (local):
#   bash tools/dist_train.sh configs/hymotion_m2m/hymotion_m2m_kimodo_uncond_046b.py 8 --auto-resume
# Launch (Taiji, 64 GPUs):
#   python tools/taiji_submit.py m2m_v2_kimodo_uncond_E3 configs/hymotion_m2m/hymotion_m2m_kimodo_uncond_046b.py --host_num 8
...
train_dataloader = dict(
    ...
    dataset=dict(
        pipeline=[
            ...
            # **KEY DIFFERENCE FROM E1/E2**: Convert SMPL Root → KIMODO Root
