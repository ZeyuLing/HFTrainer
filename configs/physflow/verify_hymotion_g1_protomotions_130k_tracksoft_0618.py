"""Soft-gated strict trackability rerun for HYMotion-G1 + ProtoMotions reward.

The proto2k run is not considered effective: fixed-noise and heldout checks show
mixed changes, not a clear improvement. This run keeps the strict tracker-quality
gate for telemetry/final judgement, but does not mask reward-SFT to zero when no
candidate clears that gate. In that case it trains toward the current best
tracker-scored candidate and reports the event as ``n_soft_fallback``.
"""

_base_ = "verify_hymotion_g1_protomotions_130k_safe.py"

work_dir = "work_dirs/physflow_verify_hymotion_g1_protomotions_130k_tracksoft_0618"
