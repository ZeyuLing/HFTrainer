#!/usr/bin/env python3
"""Merge 2026-04-30 E8-D sharded eval JSONs into importable runs."""
...
RERUN_ROOT = ROOT / "work_dirs" / "e8_d_rerun_latest_20260430"
...
def main() -> None:
    ...
    for path in sorted(RERUN_ROOT.rglob("eval_v2_*.json")):
        ...
        for model_name, model_block in nested.items():
            if not isinstance(model_block, dict):
                continue
            for _task_key, entry in model_block.get("tasks", {}).items():
                task_id = entry.get("task_id")
                setting = entry.get("setting")
                if (task_id, setting) != ("E8", "D"):
                    continue
    ...
    if not groups:
        raise SystemExit(f"No E8-D shard JSONs found under {RERUN_ROOT}")
    ...
    for (model_name, task_id, setting), group in sorted(groups.items()):
        ...
        if seen != list(range(200)):
            raise SystemExit(
                f"Expected sample_idx 0..199 for {(model_name, task_id, setting)}, got "
                f"{seen[:5]}...{seen[-5:]} (n={len(seen)})"
            )
        ...
        print(
            f"{model_name} {task_id}/{setting}: n={len(samples)} ckpt={meta['checkpoint']} "
            f"loop_err={agg.get('loop_position_error', {}).get('mean')} "
            f"foot_ratio={agg.get('foot_skating_ratio', {}).get('mean')} "
            f"foot_avg={agg.get('foot_avg_skate', {}).get('mean')}"
        )
