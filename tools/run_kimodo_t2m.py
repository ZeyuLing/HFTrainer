...
# Eval prompts — default to the same rewritten E1 datalist used by
# eval_m2m_v2_all_tasks.py when --use-rewritten is passed.
EVAL_DATA_DIR = PROJECT_ROOT / "data" / "eval" / "m2m_v2"
DEFAULT_DATA_FILE = EVAL_DATA_DIR / "eval_e1_t2m_rewritten.json"
LEGACY_RESULT_JSON = PROJECT_ROOT / "work_dirs" / "m2m_v2_t2m_eval" / "caption_global" / "result.json"
...
def load_prompts(data_file: Optional[str] = None, use_legacy_result: bool = False):
    """Load E1 prompts.

    Prefer the eval datalist so KIMODO and HyMotion consume exactly the same
    rewritten captions. The legacy result.json path is kept only for old
    reproductions.
    """
    if use_legacy_result:
        with open(LEGACY_RESULT_JSON) as f:
            data = json.load(f)
        prompts = []
    ...
    with open(path) as f:
        data = json.load(f)
    prompts = []
    items = data.get("data_list") or data.get("data") or data.get("samples") or []
    for s in items:
        prompts.append({
            "id": s.get("prompt_id") or s.get("id") or s.get("sample_id"),
            "text": _prompt_text(s),
            "frames": int(s.get("num_frames") or s.get("target_frames") or s.get("frames")),
        })
    return prompts
...
def main():
    ...
    parser.add_argument("--legacy-result-json", action="store_true",
                        help="Use the historical caption_global result.json prompt source.")
    ...
    # Load prompts
    prompts = load_prompts(args.data_file, args.legacy_result_json)
    source = LEGACY_RESULT_JSON if args.legacy_result_json else args.data_file
