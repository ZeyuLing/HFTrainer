...
V2_MODELS = {
    ...
    'caption_local_phase3_rebalanced': {
        'config': 'configs/hymotion_m2m_v2/hymotion_m2m_v2_caption_local_phase3_rebalanced.py',
        'work_dir': 'work_dirs/hymotion_m2m_v2_caption_local_phase3_rebalanced',
        'desc': 'v2 Caption + Local rotation (Phase 3: rebalanced T2M/control)',
        'has_caption': True,
        'rotation_space': 'local',
    },
    'caption_global_phase3_rebalanced': {
        'config': 'configs/hymotion_m2m_v2/hymotion_m2m_v2_caption_global_phase3_rebalanced.py',
        'work_dir': 'work_dirs/hymotion_m2m_v2_caption_global_phase3_rebalanced',
        'desc': 'v2 Caption + Global rotation (Phase 3: rebalanced T2M/control)',
        'has_caption': True,
        'rotation_space': 'global',
    },
}
        ...
        if cached is not None:
            ...
            try:
                ...
                text_fields = {
                    ...
                    'text_ctxt_raw_length': text_out['text_ctxt_raw_length'],
                }
            except Exception as e:
...
def evaluate_sample(
    ...
    keyframe_indices = None  # E3 adaptive visualization / persistence metadata
    ...
    # ---- Standard mask building ----
        ...
        if task.task_id == 'E3' and setting_name in ('adaptive', 'D'):
            ...
            e6_mask_kwargs = dict(task.settings[setting_name].mask_kwargs)
            e6_mask_kwargs.pop('contact_frames', None)
            mask = task.mask_builder(
                T, D,
    ...
    if keyframe_indices is not None:
        metrics['_keyframe_indices'] = [int(x) for x in keyframe_indices.tolist()]
    return metrics, output_135
...
def main():
    ...
    parser.add_argument('--start-index', type=int, default=0,
                        help='Start sample index after datalist filtering, inclusive')
    parser.add_argument('--end-index', type=int, default=None,
                        help='End sample index after datalist filtering, exclusive')
    parser.add_argument('--seed-offset', type=int, default=0,
                        help='Additional offset added to deterministic per-sample seed')
    ...
    parser.add_argument('--text-guidance-scale', type=float, default=2.0,
                        help='CFG scale for text-conditioned models. The v2 '
                             'caption models were trained with cond_mask_prob=0.1; '
                             'scale=5.0 is empirically OOD and causes jitter.')
    ...
    for model_name in args.models:
        ...
        for task_id in task_ids:
            ...
            for setting_name in settings:
                ...
                load_max_samples = args.max_samples
                if args.end_index is not None:
                    load_max_samples = max(load_max_samples, args.end_index)
                ...
                if args.start_index or args.end_index is not None:
                    ...
                    end_index = args.end_index if args.end_index is not None else len(samples)
                    for original_idx, _sample in enumerate(samples):
                        _sample['_eval_sample_idx'] = original_idx
                    samples = samples[start_index:end_index]
                    print(f'    Shard indices [{start_index}, {end_index}) -> '
                ...
                for i, sample in enumerate(samples):
                    try:
                        sample_idx = int(sample.get('_eval_sample_idx', i))
                        ...
                        seed = 0xE4A10000 + sample_idx + args.seed_offset
                        ...
                        if npz_dir is not None and output_135 is not None:
                            try:
                                ...
                                wp, _, _, _ = motion135_to_fk(
                                    ...
                                    rotation_space=model_info.get('rotation_space', 'local'))
                                pos_np = wp.numpy()
                                npz_path = os.path.join(npz_dir, f'{sample_idx:05d}.npz')
                                ...
                                if _layout is not None:
                                    ...
                                    _save_kw['layout_json'] = np.frombuffer(
                                        ...
                                        dtype=np.uint8)
                                _keyframes = metrics.get('_keyframe_indices', None)
                                if _keyframes is not None:
                                    _save_kw['keyframe_indices'] = np.asarray(
                                        _keyframes, dtype=np.int32)
