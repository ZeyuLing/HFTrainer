#!/usr/bin/env bash
# Submit the LEARNABILITY-FRONTIER (direction-B) co-evolution OVERFIT validation
# on AILab_DHA V100 until we land on a host whose CUDA driver is >= 11.4 (IsaacGym
# PhysX GPU pipeline). The node script's driver gate exits ~10s on a bad host;
# this loop stops that job and retries. On a good host it leaves the job running
# (3-round overfit, ~1-2h) and exits so we can tail the orchestrator log.
set -uo pipefail
...
for att in $(seq 1 "$MAXATT"); do
  log="work_dirs/physflow_frontier_overfit_a${att}.log"
  : > "$log"
  name="physflow_frontier_ovf_a${att}"
  ...
  out=$(python3 .claude/skills/taiji/taiji_ops.py submit --token "$TOKEN" -n "$name" \
    ...
    --cmd "cd $REPO && bash scripts/embodied/physflow_coevo_overfit_frontier_node.sh > $log 2>&1; echo OVF_EXIT=\$? >> $log" \
    --no-confirm 2>&1)
  ...
  if [ -n "$good" ]; then
    ...
    echo "$tf" > work_dirs/physflow_frontier_overfit_good_task.txt
