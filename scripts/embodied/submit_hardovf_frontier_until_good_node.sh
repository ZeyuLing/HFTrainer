#!/usr/bin/env bash
# Submit the AGILE hard-overfit frontier co-evolution on AILab_DHA V100 until we
# land on a host with CUDA driver >= 11.4 (IsaacGym PhysX). Stops bad-driver nodes
# fast (driver gate), retries; leaves the good-node job running (~3-4h, 5 rounds).
set -uo pipefail
...
for att in $(seq 1 "$MAXATT"); do
  log="work_dirs/physflow_hardovf_frontier_a${att}.log"
  : > "$log"
  name="physflow_hardovf_fr_a${att}"
  ...
  out=$(python3 .claude/skills/taiji/taiji_ops.py submit --token "$TOKEN" -n "$name" \
    ...
    --cmd "cd $REPO && bash scripts/embodied/physflow_coevo_hardovf_frontier_node.sh > $log 2>&1; echo HOVF_EXIT=\$? >> $log" \
    --no-confirm 2>&1)
  ...
  if [ -n "$good" ]; then
    ...
    echo "$tf" > work_dirs/physflow_hardovf_frontier_good_task.txt
