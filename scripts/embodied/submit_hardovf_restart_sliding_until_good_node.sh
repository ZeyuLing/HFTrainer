#!/usr/bin/env bash
# Submit the restart-sliding AGILE hard-overfit run until Taiji lands on a
# CUDA>=11.4 V100 node.
set -uo pipefail
...
for att in $(seq 1 "$MAXATT"); do
  log="work_dirs/physflow_hardovf_restart_sliding_a${att}.log"
  : > "$log"
  name="physflow_hovf_rslide_a${att}"
  ...
  out=$(python3 .claude/skills/taiji/taiji_ops.py submit --token "$TOKEN" -n "$name" \
    ...
    --cmd "cd $REPO && bash scripts/embodied/physflow_coevo_hardovf_restart_sliding_node.sh > $log 2>&1; echo HRSLIDE_EXIT=\$? >> $log" \
    --no-confirm 2>&1)
  ...
  if [ -n "$good" ]; then
    ...
    echo "$tf" > work_dirs/physflow_hardovf_restart_sliding_good_task.txt
