#!/usr/bin/env bash
# PhysFlow AGILE hard-overfit, LEARNABILITY-FRONTIER + GT replay + conservative
# PPO (direction-B) co-evolution on
# one Taiji node. Prompt set = 24 most-agile real clips the SOTA tracker drops, so
# a genuine frontier exists. Watch: n_frontier_mean>0 (round 1+), sel_trainee_compl
# < 0.9, and the trainee's eval completion on these clips RISING across rounds.
set -uo pipefail
REPO=/apdcephfs_cq11/share_1467498/home/zeyuling/hf_trainer
ENVDIR=/apdcephfs_cq11/share_1467498/home/zeyuling/physflow_env
cd "$REPO"
echo "[hardovf-frontier] $(date) host=$(hostname)"
...
echo "[hardovf-frontier] host CUDA driver version: ${CUDA_DRV:-unknown}"
if [ -n "$CUDA_DRV" ] && awk "BEGIN{exit !($CUDA_DRV < 11.4)}"; then
  echo "[hardovf-frontier] FATAL_BAD_NODE: CUDA driver $CUDA_DRV < 11.4. Aborting fast for reschedule."
  exit 42
fi
echo "[hardovf-frontier] driver gate OK (>=11.4)"
...
dnf install -y python38 python38-devel gcc gcc-c++ make ninja-build 2>&1 | tail -2 || echo "[hardovf-frontier] WARN dnf partial"
PY38RT="$ENVDIR/py38_runtime"
if ! /usr/bin/python3.8 -c "import encodings" >/dev/null 2>&1; then
  echo "[hardovf-frontier] restoring base python3.8 from $PY38RT"
...
/root/physflow_isaacgym_py38_cu118/bin/python -c \
  "import isaacgym, torch; print('[hardovf-frontier] isaacgym ok torch', torch.__version__)" 2>&1 | tail -3
...
timeout 300 /usr/local/bin/python3 -m pip install --quiet \
  ...
  || echo "[hardovf-frontier] WARN py310 judge dep install partial"
...
# --- 3) orchestrator: 5 rounds. Conservative PPO because direct fine-tuning of
# a converged SOTA tracker on hard/agile pool degrades without a policy anchor.
/usr/local/bin/python3 scripts/embodied/physflow_coevolve_orchestrator.py \
  --arm-name hardovf_frontier_gtreplay_cons \
  ...
  --root work_dirs/physflow_coevolve_hardovf_frontier_gtreplay_cons
echo "[hardovf-frontier] orchestrator exit=$?"
