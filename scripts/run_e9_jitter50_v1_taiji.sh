...
OUT_ROOT="${OUT_ROOT:-work_dirs/e9_jitter50_v1_full}"
DL_PATH="${DL_PATH:-data/eval/m2m_v2/eval_e9_repair_jitter50_v1.json}"
TARGET_PER_TYPE="${TARGET_PER_TYPE:-50}"
CANDIDATE_CAP="${CANDIDATE_CAP:-240}"
DASHBOARD_SETTING="${DASHBOARD_SETTING:-jitter50_v1}"
RUN_TAG="${RUN_TAG:-E9 jitter-focused 50-case repair set, 2026-04-29}"

mkdir -p "$OUT_ROOT"

echo "[stage1] build jitter datalist $(date '+%F %T')"
python3 tools/build_e9_repair_v2.py \
  ...
  --exclude-datalist data/eval/m2m_v2/eval_e9_repair_v4_nogame_temporal.json \
  --out "$DL_PATH" \
  > "$OUT_ROOT/build_e9_jitter50.log" 2>&1
