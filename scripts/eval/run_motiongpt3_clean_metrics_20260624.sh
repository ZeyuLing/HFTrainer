#!/usr/bin/env bash
# Metrics for the clean MotionGPT3 HumanML3D official-test rerun.
set -euo pipefail
...
RUN_ROOT="${RUN_ROOT:-$BASE/ms272/_suites/motiongpt3_clean_20260624_metrics}"
RESULTS="$RUN_ROOT/results"
LOGS="$RUN_ROOT/logs"
MOTION135="$BASE/motion135/motiongpt3"
MS272="$BASE/ms272/motiongpt3"
...
CAPTION_TEXTS="$BASE/captions/gt_motionclip_selected_20260622/texts"
CAPTION_ANNO="$BASE/captions/gt_motionclip_selected_20260622/test_hml3d_official272_gtlen_motionclip_selected_caption.json"
MC_REAL="$ROUNDTRIP_MC135"
MC_PRED="$RUN_ROOT/motionclip135/motiongpt3"
MC_MANIFEST="$RUN_ROOT/motionclip_manifest.tsv"

mkdir -p "$RESULTS" "$LOGS" "$MC_PRED"

echo "[start] MotionGPT3 clean metrics $(date -Is)"
...
python3 scripts/eval/eval_motionstreamer_272.py \
  ...
  --tag motiongpt3_clean_hmlroundtrip \
  --text-dir "$CAPTION_TEXTS" \
  --min-motion-len 1 \
  --out-json "$RESULTS/motiongpt3_motionstreamer272_hmlroundtrip.json" \
  2>&1 | tee "$LOGS/motionstreamer272_hmlroundtrip.log"

python3 scripts/eval/eval_motionstreamer_272.py \
  --pred-dir "$MOTION135" \
  --tag motiongpt3_clean_raw_refk \
  --real-encoding refk \
  --also-refk \
  --text-dir "$CAPTION_TEXTS" \
  --min-motion-len 1 \
  --out-json "$RESULTS/motiongpt3_motionstreamer272_raw_refk.json" \
...
python3 scripts/eval/eval_mbench_physics_dir.py \
  ...
  --out-json "$RESULTS/motiongpt3_physics.json" \
...
echo "[done] MotionGPT3 clean metrics $(date -Is)"
