#!/usr/bin/env bash
# Metrics for framework-native FlowMDM / MotionLab / MotionGPT3 HumanML3D runs.
set -euo pipefail
...
RUN_ROOT="${RUN_ROOT:-$BASE/ms272/_suites/framework_native_3methods_20260625_metrics}"
...
METHODS=(flowmdm motionlab motiongpt3)

label_for() {
  case "$1" in
    flowmdm) echo "FlowMDM" ;;
    motionlab) echo "MotionLab" ;;
    motiongpt3) echo "MotionGPT3" ;;
    *) echo "$1" ;;
  esac
}
...
echo "[start] framework-native 3-method metrics $(date -Is)"
...
base = Path("outputs/evaluation/t2m/humanml3d_official_test/ms272/_suites/framework_native_3methods_20260625_metrics")
...
echo "[done] framework-native 3-method metrics $(date -Is)"
