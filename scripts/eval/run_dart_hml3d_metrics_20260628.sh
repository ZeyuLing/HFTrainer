#!/usr/bin/env bash
# Metrics for the hftrainer-native DART HumanML3D official-test rollout.
set -euo pipefail
...
METHOD="${METHOD:-dart}"
DISPLAY_NAME="${DISPLAY_NAME:-DART}"
MOTION135="${MOTION135:-$BASE/motion135/$METHOD}"
MS272="${MS272:-$BASE/ms272/$METHOD}"
RUN_ROOT="${RUN_ROOT:-$BASE/ms272/_suites/dart_20260628}"
...
mkdir -p "$RESULTS" "$LOGS" "$MC_PRED_ROOT" "$MS272"

echo "[start] DART metrics $(date -Is)"
...
echo "[coverage] motion135_count=$count"
...
printf "%s\t%s\n" "$DISPLAY_NAME" "$mc_pred" > "$MC_MANIFEST"
...
METHOD="$METHOD" DISPLAY_NAME="$DISPLAY_NAME" RUN_ROOT="$RUN_ROOT" python3 - <<'PY'
import json
import os
...
display_name = os.environ.get("DISPLAY_NAME", "DART")
...
row = {
    ...
    "ms272_dir": f"outputs/evaluation/t2m/humanml3d_official_test/ms272/{method}",
    "ms_hmlroundtrip": {
...
msv = row["ms_hmlroundtrip"]
mcv = row["motionclip_hmlroundtrip"]
ph = row["physics"]
vals = [
    row["method"], row["samples"],
    msv["R1"], msv["R2"], msv["R3"], msv["FID"], msv["MM"], msv["Diversity"],
    mcv["R1"], mcv["R2"], mcv["R3"], mcv["FID"], mcv["MM"], mcv["Diversity"],
    ph["Slide"], ph["Float"], ph["Jitter"], ph["Dynamic"],
]
lines = ["\t".join(header), "\t".join(str(v) for v in vals)]
(base / "summary.tsv").write_text("\n".join(lines) + "\n")
print((base / "summary.tsv").read_text())
PY

echo "[done] DART metrics $(date -Is)"
