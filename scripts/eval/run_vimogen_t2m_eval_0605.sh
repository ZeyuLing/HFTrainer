...
python3 scripts/eval/build_vimogen_eval_json.py "${BUILD_JSON_ARGS[@]}" \
...
mods = ["omegaconf", "easydict", "diffusers", "transformers", "safetensors", "ftfy", "natsort", "smplx", "torchgeometry"]
missing = []
for m in mods:
    try:
        __import__(m)
...
PY
  if grep -q "missing \\[\\]" "$LOGDIR/install_check.log"; then
    ...
    python3 -m pip install -q omegaconf easydict 'diffusers>=0.31.0' 'transformers>=4.40.0' safetensors ftfy natsort smplx torchgeometry gdown \
...
CUDA_VISIBLE_DEVICES=${ENC_GPU:-0} \
python3 ./models/transformer/wan/text_encoding_batch.py \
  --json_file "$JSON_ABS" \
  --text_key prompt \
...
pushd "$VIMOGEN_ROOT_ABS" >/dev/null
python3 -m torch.distributed.run --nproc_per_node "$NPROC" --master_port "${MASTER_PORT:-29615}" \
  train_eval_vimogen.py --mode eval \
  --config "$CONFIG_ABS" \
  --mbench_name "$RUN_TAG" \
...
if [[ "$DATASET" == "mbench" ]]; then
  ...
  mkdir -p "$MBENCH_OUT"
  pushd "$VIMOGEN_ROOT_ABS" >/dev/null
  python3 scripts/organize_mbench_results.py \
...
python3 scripts/eval/eval_with_motionclip_evaluator.py \
  ...
  --rewritten_caption_file "$CAPMAP_ABS" \
