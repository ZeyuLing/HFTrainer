#!/usr/bin/env bash
# Metrics for the clean FlowMDM / MotionLab HumanML3D official-test reruns.
set -euo pipefail
...
METHODS=(flowmdm motionlab)

label_for() {
  case "$1" in
    flowmdm) echo "FlowMDM" ;;
    motionlab) echo "MotionLab" ;;
    *) echo "$1" ;;
  esac
}
...
echo "[start] FlowMDM/MotionLab clean metrics $(date -Is)"
...
for method in "${METHODS[@]}"; do
  ...
  python3 scripts/eval/eval_motionstreamer_272.py \
    ...
    --tag "${method}_clean_hmlroundtrip" \
...
summary = {
    "protocol": {
        ...
        "motionstreamer_reference": str(base.parents[2] / "_runs/noncanonical_legacy_20260623/ms272/gt_hml263_roundtrip_20260623_rootfix/predictions/ms272"),
        "motionclip_reference": str(base.parents[0] / "_suites/motionclip_gt_hml263_roundtrip_20260623/motionclip135/roundtrip"),
    },
    "rows": rows,
}
...
echo "[done] FlowMDM/MotionLab clean metrics $(date -Is)"
