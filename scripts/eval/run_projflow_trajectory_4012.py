#!/usr/bin/env python3
"""Generate ProjFlow root-trajectory/waypoint predictions on HumanML3D-4012."""
...
DEFAULT_PROTOCOL_DIR = (
    ROOT / "data/evaluation/trajectory/humanml3d_official_test_4012"
)


def trajectory_control(mode: str, length: int, waypoint_record=None) -> dict:
    if mode == "dense":
        return {"control_mode": "trajectory", "keyframe_indices": None}
    if waypoint_record is None:
        raise ValueError("sparse trajectory control requires a waypoint record")
    fractions = (
        waypoint_record.get("fracs", waypoint_record)
        if isinstance(waypoint_record, dict)
        else waypoint_record
    )
    return {
        "control_mode": "waypoints",
        "keyframe_indices": frame_indices_from_fractions(fractions, length),
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--mode", required=True, choices=("dense", "sparse"))
    parser.add_argument("--axes", required=True, choices=("xz", "xyz"))
    parser.add_argument("--data-file", type=Path, default=DEFAULT_DATA)
    parser.add_argument(
        "--ids", type=Path, default=DEFAULT_PROTOCOL_DIR / "source_ids.json"
    )
    parser.add_argument(
        "--waypoint-file",
        type=Path,
        default=DEFAULT_PROTOCOL_DIR / "sparse_waypoints.json",
    )
...
def main() -> None:
    ...
    waypoints = load_json(args.waypoint_file.resolve()) if args.mode == "sparse" else {}
    ...
    if ids:
        ...
        probe = trajectory_control(
            args.mode, min(len(probe_motion), 196), waypoints.get(ids[0])
        )
    print(
        json.dumps(
            {
    ...
    for batch in chunks(ids, args.batch_size):
        ...
        controls = [
            trajectory_control(args.mode, length, waypoints.get(motion_id))
        ...
        try:
            predictions = pipeline.infer_control(
                [caption(records[motion_id]) for motion_id in todo],
                motions,
                lengths=lengths,
