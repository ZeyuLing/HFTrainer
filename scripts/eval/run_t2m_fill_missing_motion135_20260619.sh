...
ensure_python_deps() {
  local stamp="${DEPS_STAMP:-/tmp/hftrainer_t2m_fill_missing_deps_v5.stamp}"
  if [[ -f "$stamp" ]]; then
    return 0
  fi
  local missing
...
RUN_ROOT="outputs/evaluation/t2m/humanml3d_official_test/_runs/fill_missing_motion135_20260619"
...
KIMODO_SMPLX_DEBUG_SRC="${KIMODO_SMPLX_DEBUG_SRC:-outputs/evaluation/t2m/humanml3d_official_test/multi_rep/kimodo_smplx_smpl_ms272_v100x1_20260616/debug_npz}"
...
out_anno.write_text(json.dumps({"meta": raw.get("meta", {}), "data_list": data}, indent=2))
...
for method in ("flowmdm", "motionlab", "mdm", "mld", "motiongpt3", "kimodo"):
    ...
    (missing_root / f"{method}_missing_anno.json").write_text(json.dumps({
        "meta": raw.get("meta", {}),
        "data_list": subset,
    }, indent=2))

kimodo_missing = [x.strip() for x in (missing_root / "kimodo_missing.txt").read_text().splitlines() if x.strip()]
with kimodo_corpus.open("w", encoding="utf-8") as f:
...
run_mdm() {
  ...
  "$PY_BIN" scripts/eval/mdm_infer_hml3d263.py \
    ...
    --anno_file "$MISSING_ROOT/mdm_missing_anno.json" \
...
run_mld() {
  ...
  "$PY_BIN" scripts/eval/mld_infer_hml3d263.py \
    --anno_file "$MISSING_ROOT/mld_missing_anno.json" \
    --anno_data_dir "$DATA_DIR" \
    --caption_protocol original \
...
run_motiongpt3() {
  ...
  "$PY_BIN" scripts/eval/motiongpt3_infer_hml3d263.py \
    --anno_file "$MISSING_ROOT/motiongpt3_missing_anno.json" \
    --anno_data_dir "$DATA_DIR" \
    --caption_protocol original \
...
case "$METHOD" in
  ...
  kimodo_smplx) run_shards kimodo_smplx run_kimodo_smplx; run_shards kimodo_smplx_convert run_kimodo_smplx_convert ;;
