#!/usr/bin/env bash
# Re-evaluate HY-Motion-Lite T2M outputs with row-to-column 6D conversion and
# GT first-frame root/yaw alignment for MotionCLIP evaluator reporting.
set -euo pipefail
...
run_convert h3d \
  outputs/evaluation/hylite_h3d_official0606/motionclip135 \
  "${OUT_ROOT}/h3d_row2col_yaw" \
  data/annotation/test_hml3d.json
run_convert mh_orig \
  outputs/evaluation/hylite_motionhub_origgen_orig_eval0606/motionclip135 \
  "${OUT_ROOT}/mh_orig_row2col_yaw" \
  data/annotation/test_motionhub_t2m.json
run_convert mh_rw \
  outputs/evaluation/hylite_motionhub_rw_orig_eval0606/motionclip135 \
