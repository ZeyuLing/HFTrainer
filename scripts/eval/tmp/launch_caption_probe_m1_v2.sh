...
RUN_ROOT="work_dirs/caption_e1_cfg_probe_20260525_m1_v2"
...
run_one() {
  ...
  local envkey
  envkey=$(printf '_EVAL_WORK_DIR__%s' "$model" | tr '[:lower:]' '[:upper:]')
  local out="$RUN_ROOT/${model}_${alias}_gs${scale}"
  mkdir -p "$out"
  echo "[$(date)] start model=$model alias=$alias scale=$scale gpu=$gpu envkey=$envkey" | tee "$out/launcher.log"
