#!/usr/bin/env bash
# Convert/evaluate MotionStreamer-272 T2M baselines with both evaluators.
set -euo pipefail

ROOT="/apdcephfs_cq11/share_1467498/home/zeyuling/hf_trainer"
cd "${ROOT}"
export PYTHONPATH="${ROOT}:${ROOT}/third_party:${PYTHONPATH:-}"
export PYTHONUNBUFFERED=1

OUT_ROOT=${OUT_ROOT:-outputs/evaluation/ms272_t2m_eval0605}
LOGDIR="${OUT_ROOT}/logs"
mkdir -p "${LOGDIR}"

FLOW_SRC=${FLOW_SRC:-outputs/evaluation/humanml3d/flowmdm_smpl272}
MLAB_SRC=${MLAB_SRC:-outputs/evaluation/humanml3d/motionlab_demo201_smpl272}
GT272_SRC=${GT272_SRC:-ref_repo/MotionStreamer/MotionStreamer/humanml3d_272/motion_data}

echo "[start] $(date)" | tee "${LOGDIR}/run.log"
echo "[sources] flow=${FLOW_SRC} motionlab=${MLAB_SRC} gt272=${GT272_SRC}" | tee -a "${LOGDIR}/run.log"

if [ "${SKIP_CACHE:-0}" != "1" ] && [ -f scripts/eval/_cache_272_data.sh ]; then
  bash scripts/eval/_cache_272_data.sh > "${LOGDIR}/cache_272.log" 2>&1 || true
else
  echo "[cache] skipped" | tee -a "${LOGDIR}/run.log"
fi

convert_one() {
  local tag="$1"
  local src="$2"
  local ms_npz="${OUT_ROOT}/${tag}/ms_npz"
  local mc135="${OUT_ROOT}/${tag}/motionclip135"
  mkdir -p "${ms_npz}" "${mc135}"
  echo "[convert:${tag}] $(date)" | tee -a "${LOGDIR}/run.log"
  python3 scripts/eval/convert_ms272_dir_for_t2m_eval.py \
    --src-dir "${src}" \
    --anno-file data/annotation/test_hml3d.json \
    --data-dir data/motionhub \
    --ms-npz-dir "${ms_npz}" \
    --motionclip-dir "${mc135}" \
    --align-to-gt-root \
    --workers "${CONVERT_WORKERS:-16}" \
    --overwrite \
    > "${LOGDIR}/convert_${tag}.log" 2>&1
}

convert_gt_motionclip() {
  local tag="gt_ms272"
  local mc135="${OUT_ROOT}/${tag}/motionclip135"
  mkdir -p "${mc135}"
  echo "[convert:${tag}] $(date)" | tee -a "${LOGDIR}/run.log"
  python3 scripts/eval/convert_ms272_dir_for_t2m_eval.py \
    --src-dir "${GT272_SRC}" \
    --anno-file data/annotation/test_hml3d.json \
    --data-dir data/motionhub \
    --motionclip-dir "${mc135}" \
    --only-mapped \
    --align-to-gt-root \
    --workers "${CONVERT_WORKERS:-16}" \
    --overwrite \
    > "${LOGDIR}/convert_${tag}.log" 2>&1
}

convert_one flowmdm_smpl272 "${FLOW_SRC}"
convert_one motionlab_demo201_smpl272 "${MLAB_SRC}"
convert_gt_motionclip

run_ms_eval() {
  local tag="$1"
  local gpu="$2"
  CUDA_VISIBLE_DEVICES="${gpu}" python3 scripts/eval/eval_motionstreamer_272.py \
    --pred-dir "${OUT_ROOT}/${tag}/ms_npz" \
    --tag "${tag}" \
    --out-json "${OUT_ROOT}/${tag}/motionstreamer272.json" \
    > "${LOGDIR}/ms_${tag}.log" 2>&1
}

run_mc_eval() {
  local tag="$1"
  local gpu="$2"
  CUDA_VISIBLE_DEVICES="${gpu}" python3 scripts/eval/eval_with_motionclip_evaluator.py \
    --evaluator_ckpt checkpoints/motion_clip/motionclip_base_1p_aug_hq \
    --anno_file data/annotation/test_hml3d.json \
    --data_dir data/motionhub \
    --pred_dir "${OUT_ROOT}/${tag}/motionclip135" \
    --rewritten_caption_file data/annotation/test_hml3d_rewritten.json \
    --chunk_size 64 \
    --out_json "${OUT_ROOT}/${tag}/motionclip_c64.json" \
    --n_repeats 20 \
    --seed 42 \
    > "${LOGDIR}/mc_${tag}.log" 2>&1
}

echo "[eval] $(date)" | tee -a "${LOGDIR}/run.log"
run_ms_eval flowmdm_smpl272 0 &
run_ms_eval motionlab_demo201_smpl272 1 &
run_mc_eval flowmdm_smpl272 2 &
run_mc_eval motionlab_demo201_smpl272 3 &
run_mc_eval gt_ms272 4 &
wait

python3 - <<PY | tee "${OUT_ROOT}/summary.txt"
import json
from pathlib import Path
root = Path("${OUT_ROOT}")
for tag in ["gt_ms272", "flowmdm_smpl272", "motionlab_demo201_smpl272"]:
    print("\\n" + tag)
    mc = root / tag / "motionclip_c64.json"
    if mc.exists():
        d = json.load(open(mc))
        print(
            " MotionCLIP",
            "samples", d.get("samples"),
            "R1", f"{d.get('r_precision_pred_top1_mean', float('nan')):.4f}",
            "R3", f"{d.get('r_precision_pred_top3_mean', float('nan')):.4f}",
            "FID", f"{d.get('fid_mean', float('nan')):.4f}",
            "MM", f"{d.get('mm_dist_pred_mean', float('nan')):.4f}",
            "Div", f"{d.get('diversity_pred_mean', float('nan')):.4f}",
        )
    ms = root / tag / "motionstreamer272.json"
    if ms.exists():
        d = json.load(open(ms))
        pred = d.get("pred", {})
        print(
            " MS272",
            "ids", d.get("ids_with_required_files"),
            "R1", f"{pred.get('r_precision', [float('nan')])[0]:.4f}",
            "R3", f"{pred.get('r_precision', [0, 0, float('nan')])[2]:.4f}",
            "FID", f"{pred.get('fid_vs_gt_native', float('nan')):.4f}",
            "MM", f"{pred.get('matching_score', float('nan')):.4f}",
            "Div", f"{pred.get('diversity', float('nan')):.4f}",
        )
PY

touch "${OUT_ROOT}/_DONE"
echo "[done] $(date)" | tee -a "${LOGDIR}/run.log"
