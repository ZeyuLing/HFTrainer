#!/usr/bin/env python3
"""A/B test: compare E14 inference with different transition pad strategies.

Strategy A (baseline): zeros pad (current behavior)
Strategy B (lerp):     linear interpolation between cond_a[-1] and cond_b[0]
Strategy C (lerp+ground): lerp + ground-anchored canonicalization
...
# Cases with severe foot skating (fs_ratio > 0.20)
SEVERE_PIDS = [0, 33, 20, 40, 48, 73, 45, 55, 34, 80, 86, 53, 88, 38, 97, 14]
...
def build_input(mA, motion_b_world, N_cond_a, N_cond_b, N_transition,
                bone_offsets, pad_strategy='zeros'):
    """Build canonical input segment for E14.

    pad_strategy: 'zeros' | 'lerp' | 'lerp_ground'
    """
    a_tail = mA[-N_cond_a:]
    b_head = motion_b_world[:N_cond_b]

    if pad_strategy == 'zeros':
        transition_pad = np.zeros((N_transition, 135), dtype=np.float32)
    elif pad_strategy in ('lerp', 'lerp_ground'):
        # Linear interpolation between last frame of A and first frame of B
        start_frame = a_tail[-1]  # (135,)
        end_frame = b_head[0]     # (135,)
        t_vals = np.linspace(0, 1, N_transition + 2)[1:-1]  # exclude endpoints
        transition_pad = np.outer(1 - t_vals, start_frame) + np.outer(t_vals, end_frame)
        transition_pad = transition_pad.astype(np.float32)
    else:
    ...
    use_ground = pad_strategy == 'lerp_ground'
    if use_ground:
...
def main():
    ...
    strategies = ['zeros', 'lerp', 'lerp_ground']
    header = f"{'pid':>5}"
    for s in strategies:
        header += f"  {s:>12}"
    print(header)
    ...
    for pid in SEVERE_PIDS:
        ...
        for strategy in strategies:
            ...
            slide = compute_slide_ratio(out_world, bone_offsets, nc_a, nc_a + n_trans)
            results[strategy].append(slide)
            row += f'  {slide:12.1%}'

        # Mark best
        vals = [results[s][-1] for s in strategies]
        best_idx = np.argmin(vals)
        print(row + f'  best={strategies[best_idx]}')

    # Summary
    print(f'\n{"Strategy":>12}  {"Mean":>8}  {"Median":>8}  {"Max":>8}')
    print('-' * 44)
    for s in strategies:
        arr = np.array(results[s])
        print(f'{s:>12}  {arr.mean():8.1%}  {np.median(arr):8.1%}  {arr.max():8.1%}')
