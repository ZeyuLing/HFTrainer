...
def repair_single_file(file_path, gpu_id, output_dir, mode='denoise', step=10):
    ...
    try:
        ...
        # 运行修复命令 - 使用正确的参数格式
        # 由于 mogendit_cjgame_eval.py 需要目录而不是单个文件，
        # 我们创建一个临时目录包含单个文件
        with tempfile.TemporaryDirectory() as tmpdir:
            # 在临时目录中创建符号链接
            tmp_input_dir = Path(tmpdir) / 'input'
            tmp_input_dir.mkdir()
            tmp_file = tmp_input_dir / input_path.name
            os.symlink(str(input_path.resolve()), str(tmp_file))
            ...
            cmd = [
                'python', 'scripts/mogendit_cjgame_eval.py',
                '--input-dir', str(tmp_input_dir),
                '--output-dir', str(tmp_output_dir),
                '--max-samples', '1',
                '--model-name', 'MoreDiff-0.1B',
                '--device', f'cuda:{gpu_id}',
                '--mode', mode,
                '--denoise-step', str(step)
            ]
            ...
            if result.returncode == 0:
                repaired_files = list(tmp_output_dir.rglob('*.npz'))
                if repaired_files:
                    repaired_file = repaired_files[0]
                    repaired_file.rename(output_path)
                    logger.info(f'GPU{gpu_id}: 成功修复 {input_path.name}')
                    return {'file': file_path, 'status': 'success', 'gpu': gpu_id}
                else:
                    logger.error(f'GPU{gpu_id}: 修复成功但未生成输出文件 {input_path.name}')
                    return {'file': file_path, 'status': 'error', 'gpu': gpu_id, 'error': 'No output file generated'}
            else:
                logger.error(f'GPU{gpu_id}: 修复失败 {input_path.name}: {result.stderr}')
                return {'file': file_path, 'status': 'failed', 'gpu': gpu_id, 'error': result.stderr}
        ...
        logger.error(f'GPU{gpu_id}: 修复超时 {file_path}')
        return {'file': file_path, 'status': 'timeout', 'gpu': gpu_id}
    except Exception as e:
        logger.error(f'GPU{gpu_id}: 修复异常 {file_path}: {e}')
        return {'file': file_path, 'status': 'error', 'gpu': gpu_id, 'error': str(e)}

def distribute_files_to_gpus(files, num_gpus):
    """将文件均匀分配到各个GPU"""
    gpu_files = [[] for _ in range(num_gpus)]

    for i, file_path in enumerate(files):
        gpu_id = i % num_gpus
        gpu_files[gpu_id].append(file_path)

    # 打印分配情况
    for gpu_id, files_list in enumerate(gpu_files):
        logger.info(f'GPU{gpu_id}: 分配 {len(files_list)} 个文件')
...
def main():
    parser = argparse.ArgumentParser(description='MoGenDIT 多GPU并行修复')
    ...
    # 保存报告
    report_file = Path(args.output_dir) / 'multigpu_repair_report.json'
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)

    # 打印总结
    logger.info('='*60)
    logger.info('多GPU修复任务完成!')
