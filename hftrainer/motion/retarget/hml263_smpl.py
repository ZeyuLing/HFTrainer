...
DEFAULT_SMPL_MODEL_DIR = REPO_ROOT / "ref_repo" / "MDM" / "body_models_nochumpy"
...
def load_smpl_rest(
    ...
    Args:
        model_dir: SMPL model directory. Defaults to
            ``ref_repo/MDM/body_models_nochumpy`` under the repository root.
        device: Torch device for the SMPL layer.
    """

    return _load_smpl_rest(Path(model_dir) if model_dir else DEFAULT_SMPL_MODEL_DIR, _as_device(device))
