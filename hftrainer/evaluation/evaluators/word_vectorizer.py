"""GloVe word vectorizer for the HumanML3D-263 evaluator (Guo et al. ``our_vab``).

Framework-internal port (no ``ref_repo`` dependency). Loads the GloVe ``our_vab``
artifacts (``*_data.npy`` / ``*_idx.pkl`` / ``*_words.pkl``) from ``checkpoints/``.
"""
...
POS_enumerator = {
    "VERB": 0, "NOUN": 1, "DET": 2, "ADP": 3, "NUM": 4, "AUX": 5, "PRON": 6,
    "ADJ": 7, "ADV": 8, "Loc_VIP": 9, "Body_VIP": 10, "Obj_VIP": 11,
...
Act_list = ("walk", "run", "swing", "pick", "bring", "kick", "put", "squat",
            ...
            "spin", "spread", "climb")
Desc_list = ("slowly", "carefully", "fast", "careful", "slow", "quickly",
             "happy", "angry", "sad", "happily", "angrily", "sadly")

VIP_dict = {
    "Loc_VIP": Loc_list, "Body_VIP": Body_list, "Obj_VIP": Obj_List,
    "Act_VIP": Act_list, "Desc_VIP": Desc_list,
}


class WordVectorizer(object):
    def __init__(self, meta_root: str, prefix: str = "our_vab"):
        vectors = np.load(pjoin(meta_root, f"{prefix}_data.npy"))
        with open(pjoin(meta_root, f"{prefix}_words.pkl"), "rb") as f:
            words = pickle.load(f)
        with open(pjoin(meta_root, f"{prefix}_idx.pkl"), "rb") as f:
            self.word2idx = pickle.load(f)
        self.word2vec = {w: vectors[self.word2idx[w]] for w in words}

    def _get_pos_ohot(self, pos: str) -> np.ndarray:
        pos_vec = np.zeros(len(POS_enumerator))
        pos_vec[POS_enumerator.get(pos, POS_enumerator["OTHER"])] = 1
        return pos_vec
