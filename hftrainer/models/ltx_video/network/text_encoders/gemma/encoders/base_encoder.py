...
from hftrainer.models.ltx_video.network.text_encoders.gemma.tokenizer import LTXGemmaTokenizer, PaddingSide
...
def _cat_with_padding(
    ...
    Decoder-only generation requires left padding so the final non-padding
    position remains the continuation point.
    """
    pad = torch.full(
        (1, padding_length),
        value,
...
def build_gemma_tokenizer(assets: GemmaAssets) -> LTXGemmaTokenizer:
    return LTXGemmaTokenizer(
        build_local_gemma_tokenizer(assets),
        TOKENIZER_MAX_LENGTH,
...
def module_ops_from_gemma_root(gemma_root: str) -> tuple[ModuleOps, ...]:
    ...
    def load_processor(module: LTXGemmaTextEncoder) -> LTXGemmaTextEncoder:
        ...
        module.processor = build_local_gemma_processor(assets, module.tokenizer.tokenizer)
        return module
