...
from hftrainer.models.ltx_video.network.loader import KeyValueOperationResult, parse_model_version
...
from hftrainer.models.ltx_video.network.text_encoders.gemma.embeddings_connector import (
    AudioEmbeddings1DConnectorConfigurator,
    Embeddings1DConnectorConfigurator,
)
from hftrainer.models.ltx_video.network.text_encoders.gemma.embeddings_processor import EmbeddingsProcessor
from hftrainer.models.ltx_video.network.text_encoders.gemma.encoders.base_encoder import LTXGemmaTextEncoder, module_ops_from_gemma_root
from hftrainer.models.ltx_video.network.text_encoders.gemma.feature_extractor import (
    FeatureExtractorV1,
    FeatureExtractorV2,
)
from hftrainer.models.ltx_video.network.text_encoders.gemma.gemma_assets import GemmaAssets, build_local_gemma_config, resolve_gemma_weight_paths
...
def _load_gemma_config(gemma_model_path: str) -> LocalGemmaConfig:
    """Load the Hugging Face config from a Gemma directory or single-file TE checkpoint."""
    return build_local_gemma_config(GemmaAssets.load(gemma_model_path))
...
class GemmaTextEncoderConfigurator(ModelConfigurator[LTXGemmaTextEncoder]):
    ...
    def from_metadata(cls, metadata: dict) -> LTXGemmaTextEncoder:  # noqa: ARG003
        ...
        with torch.device("meta"):
            model = build_local_gemma_model(gemma_config)
...
def _check_gemma_version(metadata: dict, gemma_config: LocalGemmaConfig, gemma_model_path: str) -> None:
...
def _create_feature_extractor(transformer_config: dict, gemma_text_config: object) -> torch.nn.Module:
    ...
    ``gemma_text_config`` is a local parsed config; only
...
def _build_gemma3_llm_key_ops() -> SDOps:
    ...
    model it lands at model.model.vision_tower.encoder.X. The reference v5 layout flattened
...
def _populate_rotary_v5(l_model: torch.nn.Module, config: object) -> None:
    """Pinned layout: one rotary module with per-layer-type buffers.
