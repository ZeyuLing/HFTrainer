...
The encoder is built first through HFTrainer's local loading path and quantized here,
rather than being loaded by another model framework. LTX 2.5 ships a
``gemma4_unified`` text encoder and a
single packed ``.safetensors`` rather than a Hugging Face directory. Both are invisible
to generic pretrained-model loaders.
...
Example usage:
    from hftrainer.models.ltx_video.component_loader import load_text_encoder
...
def quantize_gemma_to_8bit(
    ...
    Args:
        text_encoder: Encoder built by :func:`hftrainer.models.ltx_video.component_loader.load_text_encoder`,
