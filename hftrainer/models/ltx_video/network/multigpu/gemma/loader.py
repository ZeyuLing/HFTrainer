...
``hftrainer.pipelines.ltx_video.backend.utils.blocks`` but uses ``device_map="auto"`` instead of
...
from hftrainer.models.ltx_video.network.text_encoders.gemma.encoders.base_encoder import LTXGemmaTextEncoder, build_gemma_tokenizer
from hftrainer.models.ltx_video.network.text_encoders.gemma.encoders.encoder_configurator import get_gemma_ops
...
from hftrainer.models.ltx_video.network.text_encoders.gemma.local_model import build_local_gemma_model
...
def load_gemma_with_device_map(
    ...
    processor = build_local_gemma_processor(assets, tokenizer.tokenizer)
...
def _load_single_file_with_device_map(assets: GemmaAssets, dtype: torch.dtype) -> torch.nn.Module:
    config = build_local_gemma_config(assets)
