...
from hftrainer.models.ltx_video.network.model.transformer.attention import AttentionCallable, MaskedAttentionCallable
...
class _TorchAll2All:
    """Repository-local sequence/head all-to-all implemented with PyTorch.

    The original fast path delegated this layout transform to a separately
    required a separately built CUDA helper. Keeping the collective here makes the
    multi-GPU execution graph self-contained.  It is intentionally a clear
    correctness implementation; optimized kernels can only be added later as
    repository-owned alternatives behind the same contract.
    """
    ...
    def __init__(
        ...
        **_: object,
        ...
        self.group = group
        self.rank_tokens = [seqlen] * world_size
        self.timeout_seconds = _DEFAULT_ALL2ALL_TIMEOUT_SECONDS

    def set_rank_tokens(self, seqlens: list[int]) -> None:
        if len(seqlens) != self.world_size or any(value < 0 for value in seqlens):
            raise ValueError(
                f'Expected {self.world_size} non-negative sequence lengths; '
                f'got {seqlens}.'
            )
        self.rank_tokens = list(seqlens)

    def set_timeout_seconds(self, seconds: float) -> None:
        if seconds <= 0:
            raise ValueError('all-to-all timeout must be positive.')
        # PyTorch ProcessGroup owns the actual timeout. Retain this value so
        # callers keep the same public control surface and validation.
        self.timeout_seconds = float(seconds)

    def send_recv_heads(self, tensor: torch.Tensor, *, copy_out: bool = False) -> torch.Tensor:
        if self.world_size == 1:
            return tensor.clone() if copy_out else tensor
        if tensor.shape[-2] % self.world_size:
            raise ValueError(
                f'Head count {tensor.shape[-2]} must be divisible by '
                f'world size {self.world_size}.'
            )
        if tensor.shape[1] != self.rank_tokens[self.rank]:
            raise ValueError(
                f'Rank {self.rank} received {tensor.shape[1]} tokens; '
                f'expected {self.rank_tokens[self.rank]}.'
            )
        local_heads = tensor.shape[-2] // self.world_size
        sends = [chunk.contiguous() for chunk in tensor.split(local_heads, dim=-2)]
        receives = [
            torch.empty(
                tensor.shape[0],
                tokens,
        ...
        dist.all_to_all(receives, sends, group=self.group)
        return torch.cat(receives, dim=1)

    def gather_heads(self, tensor: torch.Tensor, *, copy_out: bool = False) -> torch.Tensor:
        if self.world_size == 1:
            return tensor.clone() if copy_out else tensor
        if tensor.shape[1] != sum(self.rank_tokens):
            raise ValueError(
                f'Gather input has {tensor.shape[1]} tokens; expected '
                f'{sum(self.rank_tokens)}.'
            )
        sends = [chunk.contiguous() for chunk in tensor.split(self.rank_tokens, dim=1)]
        receives = [
            torch.empty(
                tensor.shape[0],
                self.rank_tokens[self.rank],
                tensor.shape[-2],
                tensor.shape[-1],
                dtype=tensor.dtype,
                device=tensor.device,
        ...
        dist.all_to_all(receives, sends, group=self.group)
        return torch.cat(receives, dim=-2)
