...
CHUNKED modes wire ``decoder.upsamples[3]`` onto each diffusion block for
deferred stage-4 (upsample Linear then ``context_proj``).
"""
...
from hftrainer.models.ltx_video.network.model.video_vae.transformer.blocks import DiffusionNABlock
...
if TYPE_CHECKING:
    from hftrainer.models.ltx_video.network.model.video_vae.video_vae import DiffusionVideoDecoder
...
def resolve_attention_for_host(cfg: DiffVAEConfig) -> DiffVAEConfig:
    ...
    if cfg.block is DiffVAEBlockKind.COMBINED:
        raise RuntimeError(
            ...
            "(full-volume NA + torch.compile). Install a NATTEN build compatible "
            "with your PyTorch/CUDA stack, or use chunked_eager / "
            "chunked_compile, which fall back to local Triton/eager code."
        )
...
def apply_diffvae_config(
    ...
    *,
    ...
    deferred = cfg.block is DiffVAEBlockKind.CHUNKED
...
def _diffusion_decoder_matcher(model: torch.nn.Module) -> bool:
    from hftrainer.models.ltx_video.network.model.video_vae.video_vae import DiffusionVideoDecoder  # noqa: PLC0415
