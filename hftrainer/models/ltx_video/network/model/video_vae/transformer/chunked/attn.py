...
The W-chunk loop stays behind opaque ``hftrainer_ltx_video::na_residual_w_chunked`` so Dynamo
...
def _resolve_chunked_attention() -> tuple[NAAttentionCallable, NeighborhoodAttention3D]:
    ...
    if fn is None or mod is None:
        raise RuntimeError(
            "hftrainer_ltx_video::na_residual_w_chunked requires attention_function binding; "
...
@torch.library.custom_op("hftrainer_ltx_video::na_residual_w_chunked", mutates_args=("x",))
...
def chunked(
    ...
    Opaque to Dynamo via ``hftrainer_ltx_video::na_residual_w_chunked``; NA uses
