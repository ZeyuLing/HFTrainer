...
from hftrainer.models.ltx_video.network.model.video_vae.transformer.det_attn_rope import det_qkv_rope
from hftrainer.models.ltx_video.network.model.video_vae.transformer.qkv import QKVProjections
from hftrainer.models.ltx_video.network.model.video_vae.transformer.rope_math import (
    ...
    def __call__(
        ...
        if not _NATTEN_AVAILABLE:
            raise ImportError(
                ...
                "Install a NATTEN build compatible with your PyTorch/CUDA stack, "
                "or select a chunked mode that can use the local Triton/eager fallback."
            )
...
class NeighborhoodAttention3D(nn.Module):
    ...
    default; Triton or eager SDPA when NATTEN is missing). Relative
    gather-based NA is not used as a
