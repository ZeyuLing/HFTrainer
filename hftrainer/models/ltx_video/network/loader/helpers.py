...
from hftrainer.models.ltx_video.network.loader.primitives import StateDict, StateDictLoader
...
def parse_model_version(version: str | None) -> tuple[int, ...]:
    ...
    separator first -- see the repository-local ``detect_params`` helper.
    """
    if not version:
        return ()
    numeric_parts = []
