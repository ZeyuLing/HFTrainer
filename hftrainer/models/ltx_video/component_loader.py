...
"""
Repository-owned loader for LTX-2 model components.
This module provides the shared model-layer construction interface used by
training, preprocessing, and inference-adjacent validation code.
Example usage:
...
if TYPE_CHECKING:
    ...
    from hftrainer.models.ltx_video.network.components.schedulers import LTX2Scheduler
    from hftrainer.models.ltx_video.network.model.audio_vae import AudioDecoder, AudioEncoder, Vocoder
    from hftrainer.models.ltx_video.network.model.transformer import LTXModel
    from hftrainer.models.ltx_video.network.model.video_vae import VideoDecoder, VideoEncoder
    from hftrainer.models.ltx_video.network.text_encoders.gemma import LTXGemmaTextEncoder
    from hftrainer.models.ltx_video.network.text_encoders.gemma.embeddings_processor import EmbeddingsProcessor
    from hftrainer.models.ltx_video.network.types import SpatioTemporalScaleFactors


class LTXComponentStore:
    """Own the model registries used by one LTX model boundary.

    Inference may reuse immutable model shells between pipeline stages. Training
    receives an exclusive, non-caching registry so a mutable transformer can
    never alias an inference shell or another trainer instance. Keeping both
    policies here makes component ownership explicit and prevents loaders from
    silently constructing their own private caches.
    """

    def __init__(self, *, inference_registry=None, training_registry=None) -> None:
        from hftrainer.models.ltx_video.network.loader.registry import ModelRegistry

        self.inference_registry = (
            inference_registry
            if inference_registry is not None
            else ModelRegistry(cache_models=True, cache_weights=False)
        )
        self.training_registry = (
            training_registry
            if training_registry is not None
            else ModelRegistry(cache_models=False, cache_weights=False)
        )

    def clear(self) -> None:
        """Release all model shells and tensors owned by this boundary."""

        self.inference_registry.clear()
        if self.training_registry is not self.inference_registry:
            self.training_registry.clear()
...
def read_video_scale_factors(model_path: str | Path) -> "SpatioTemporalScaleFactors":
    ...
    """
    from hftrainer.models.ltx_video.network.loader import SafetensorsModelStateDictLoader
    from hftrainer.models.ltx_video.network.types import SpatioTemporalScaleFactors
...
def embedding_weight_paths(
    ...
    """
    from hftrainer.models.ltx_video.network.text_encoders.gemma.gemma_assets import is_safetensors_file
...
def load_video_vae_encoder(
    ...
    """
    from hftrainer.models.ltx_video.network.loader.single_gpu_model_builder import SingleGPUModelBuilder
    from hftrainer.models.ltx_video.network.model.video_vae import VAE_ENCODER_COMFY_KEYS_FILTER, VideoEncoderConfigurator
...
def load_video_vae_decoder(
    ...
    from hftrainer.models.ltx_video.network.model.video_vae.transformer import build_cutlass_fna_diffusion_decoder_op
...
def load_audio_vae_encoder(
    ...
    Returns:
        Loaded AudioEncoder
    """
    from hftrainer.models.ltx_video.network.loader import SingleGPUModelBuilder
    from hftrainer.models.ltx_video.network.model.audio_vae import AUDIO_VAE_ENCODER_COMFY_KEYS_FILTER, AudioEncoderConfigurator
...
def load_audio_vae_decoder(
    ...
    """
    from hftrainer.models.ltx_video.network.loader import SingleGPUModelBuilder
    from hftrainer.models.ltx_video.network.model.audio_vae import AUDIO_VAE_DECODER_COMFY_KEYS_FILTER, AudioDecoderConfigurator
...
def load_vocoder(
    ...
    """
    from hftrainer.models.ltx_video.network.loader import SingleGPUModelBuilder
    from hftrainer.models.ltx_video.network.model.audio_vae import VOCODER_COMFY_KEYS_FILTER, VocoderConfigurator
...
def load_text_encoder(
    ...
    """
    from hftrainer.models.ltx_video.network.loader.single_gpu_model_builder import SingleGPUModelBuilder
    from hftrainer.models.ltx_video.network.text_encoders.gemma import (
        GemmaTextEncoderConfigurator,
        get_gemma_ops,
    )
    from hftrainer.models.ltx_video.network.text_encoders.gemma.gemma_assets import resolve_gemma_weight_paths
    ...
    if load_in_8bit:
        from hftrainer.models.ltx_video.network.text_encoders.gemma.quantization import (
            quantize_gemma_to_8bit,
        )
...
def load_model(  # noqa: PLR0913
    ...
    transformer = load_transformer(
        checkpoint_path, torch_device, dtype, registry=registry
    )
    ...
    if with_video_vae_encoder:
        ...
        video_vae_encoder = load_video_vae_encoder(
            video_vae_path, torch_device, dtype, registry=registry
        )
    ...
    if with_audio_vae_decoder:
        ...
        audio_vae_decoder = load_audio_vae_decoder(
            audio_vae_path, torch_device, dtype, registry=registry
        )
    ...
    if with_vocoder:
        ...
        vocoder = load_vocoder(audio_vae_path, torch_device, dtype, registry=registry)
    ...
    if with_text_encoder:
        ...
        text_encoder = load_text_encoder(
            text_encoder_path, torch_device, dtype, registry=registry
        )
