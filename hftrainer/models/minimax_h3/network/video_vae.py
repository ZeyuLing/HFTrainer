...
class _SwiGLU(nn.Module):
    """Diffusers-compatible SwiGLU, including checkpoint key ``proj``."""
...
class FeedForward(nn.Module):
    """Minimal local equivalent of Diffusers' SwiGLU feed-forward block."""
    ...
    def __init__(
        ...
        if activation_fn != "swiglu":
            raise ValueError(
                f"Only the MiniMax-H3 `swiglu` activation is supported, got {activation_fn!r}."
            )
...
class MiniMaxH3VideoDownsample3d(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if self.spatial_stride == 2:
            hidden_states = F.pad(
                hidden_states, (0, 1, 0, 1, 0, 0), mode=self.spatial_padding_mode
            )
...
class MiniMaxH3VideoRotaryPosEmbed(nn.Module):
    ...
    def _make_inv_freq(self, device: torch.device | str | None = None) -> torch.Tensor:
        return 1.0 / self.theta ** torch.arange(
            0,
            1,
    ...
    def materialize(self, device: torch.device | str) -> None:
        """Recreate the deterministic non-persistent table after meta loading."""
...
class MiniMaxH3VideoAttention(nn.Module):
    ...
    def __init__(
        self, dim: int, heads: int, dim_head: int, eps: float = 1e-5, bias: bool = True
    ) -> None:
        super().__init__()
        self.heads = heads
...
class MiniMaxH3VideoTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = (
            hidden_states + self.attn(norm_hidden_states, rotary_emb) * self.scale1
        )
        ...
        for block in self.transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                def checkpointed_block(
                    ...
                    # Bind the current layer explicitly.  Checkpoint recomputes
                    # this closure during backward, after the loop has moved on
                    # to later blocks.
                    return current_block(value, (cos_value, sin_value))
...
class AutoencoderKLMiniMaxH3(LocalMiniMaxH3ModelMixin, nn.Module):
    r"""
    ...
    def __init__(
        ...
        self.config = ConfigDict(
            {
                ...
                "spatial_downsample_factors": list(spatial_downsample_factors),
                "temporal_downsample_factors": list(temporal_downsample_factors),
                "norm_num_groups": int(norm_num_groups),
                "norm_eps": float(norm_eps),
                "spatial_padding_mode": str(spatial_padding_mode),
        ...
        self.tokens_chunk_size = math.ceil(
            clip_length / self.temporal_compression_ratio
        )
    ...
    def _materialize_nonpersistent_buffers(self, device: torch.device | str) -> None:
        """Restore deterministic buffers omitted from checkpoint state dicts."""

        for module in self.modules():
            if (
                isinstance(module, MiniMaxH3VideoRotaryPosEmbed)
                and module.inv_freq.is_meta
            ):
    ...
    def set_attn_processor(self, processor) -> None:
        for module in self.modules():
            if isinstance(module, MiniMaxH3VideoAttention):
