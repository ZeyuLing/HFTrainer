...
#
# MODIFIED BY HFTRAINER: repository-local PyTorch implementation with no
# runtime dependency on diffusers, transformers, or peft.
...
from .common import LocalMiniMaxH3ModelMixin, get_parameter_dtype, randn_tensor
from .configuration import ConfigDict
from .outputs import DecoderOutput, MiniMaxH3AudioEncoderOutput
...
class MiniMaxH3AudioAttnProcessor:
    ...
    *bidirectional* attention, and context parallelism, which raises for causal attention.
    ...
    def __call__(
        self, attn: "MiniMaxH3AudioCausalAttention", hidden_states: torch.Tensor
    ) -> torch.Tensor:
        ...
        query, key, value = (
            qkv.reshape(batch_size, seq_len, 3, attn.num_heads, attn.head_dim)
            .permute(2, 0, 1, 3, 4)
            .unbind(0)
        )
...
class AutoencoderKLMiniMaxH3Audio(LocalMiniMaxH3ModelMixin, nn.Module):
    r"""
    ...
    def __init__(
        ...
        self.config = ConfigDict(
            {
                ...
                "decoder_kernel_sizes": [int(value) for value in decoder_kernel_sizes],
                "resblock_kernel_sizes": [
                    int(value) for value in resblock_kernel_sizes
                ],
                "resblock_dilation_sizes": [
                    [int(value) for value in values]
                    for values in resblock_dilation_sizes
                ],
                "sampling_rate": int(sampling_rate),
                "latents_mean": None
                if latents_mean is None
                else [float(value) for value in latents_mean],
                "latents_std": None
                if latents_std is None
                else [float(value) for value in latents_std],
            }
        )

        self.encoder = MiniMaxH3AudioEncoder(
            d_model=encoder_dim, strides=encoder_rates, d_latent=latent_dim
        )
        self.pre_block = MiniMaxH3AudioAttnProjection(
            latent_dim, latent_channels, num_heads=num_attention_heads
        )
        ...
        self.decoder = MiniMaxH3AudioBigVGANDecoder(
            ...
            resblock_kernel_sizes=tuple(
                int(kernel) for kernel in resblock_kernel_sizes
            ),
            resblock_dilation_sizes=tuple(
                tuple(int(d) for d in dilation) for dilation in resblock_dilation_sizes
            ),
        )
    ...
    def set_attn_processor(self, processor) -> None:
        for module in self.modules():
            if isinstance(module, MiniMaxH3AudioCausalAttention):
    ...
    def encode(
        ...
        if sample.ndim != 3 or sample.shape[1] != 1:
            raise ValueError(
                f"`sample` must have shape [batch_size, 1, samples], got {tuple(sample.shape)}."
            )

        right_pad = (
            math.ceil(sample.shape[-1] / self.hop_length) * self.hop_length
            - sample.shape[-1]
        )
        if right_pad > 0:
        ...
        if self.gradient_checkpointing and self.training and torch.is_grad_enabled():
            hidden_states = checkpoint(self.encoder, sample, use_reentrant=False)
            hidden_states = checkpoint(
                lambda value: self.pre_block(value.transpose(1, 2)).transpose(1, 2),
                hidden_states,
                use_reentrant=False,
            )
            mean = checkpoint(self.mean_proj, hidden_states, use_reentrant=False)
            logs = checkpoint(self.logs_proj, hidden_states, use_reentrant=False)
        else:
            hidden_states = self.encoder(sample)
            hidden_states = self.pre_block(hidden_states.transpose(1, 2)).transpose(
                1, 2
            )
    ...
    def decode(
        ...
        if self.gradient_checkpointing and self.training and torch.is_grad_enabled():
            hidden_states = checkpoint(self.dec_in_proj, latents, use_reentrant=False)
            decoded = checkpoint(self.decoder, hidden_states, use_reentrant=False)
        else:
