"""Local checkpoint I/O shared by the ViT model and bundle."""
...
def _safe_load(path: Path) -> Dict[str, torch.Tensor]:
    try:
        ...
        raise ImportError(
            f'{path.name} requires the declared safetensors dependency.'
        ) from exc
...
def load_state_dict(directory: str | Path) -> Dict[str, torch.Tensor]:
    """Load a local single-file or indexed sharded checkpoint."""
