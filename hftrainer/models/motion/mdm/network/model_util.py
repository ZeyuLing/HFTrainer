"""MDM model + diffusion factory for hftrainer.

The MDM network lives in :mod:`.network`, the Gaussian diffusion in
:mod:`.diffusion`, and the few constants/helpers are inlined below. The model
hyper-parameters and diffusion schedule are kept identical to the released
checkpoint contract.
"""
...
# Inlined from MDM ``data_loaders/humanml_utils.py`` (only the end-effector names
# are referenced, and only for the unused multi-target-cond path).
HML_EE_JOINT_NAMES = ["left_foot", "right_foot", "left_wrist", "right_wrist", "head"]


def get_cond_mode(args):
    """Inlined from MDM ``utils/parser_util.get_cond_mode``."""
...
def load_model_wo_clip(model, state_dict):
    # The positional-encoding buffers are fixed (not learned) and may differ in
    # length across checkpoints, so drop them before loading.
    ...
    assert all(
        k.startswith("clip_model.") or "sequence_pos_encoder" in k for k in missing_keys
...
def get_model_args(args, data):
    ...
    num_actions = getattr(getattr(data, "dataset", None), "num_actions", 1)
    ...
    return {
        "modeltype": "", "njoints": njoints, "nfeats": nfeats, "num_actions": num_actions,
...
def create_gaussian_diffusion(args):
    # MDM always predicts x_start with a fixed (small) posterior variance and no
    # learned sigma; the full schedule keeps all `diffusion_steps` (no respacing).
    predict_xstart = True
    ...
    lambda_target_loc = getattr(args, "lambda_target_loc", 0.0)
...
def load_saved_model(model, model_path, use_avg: bool = False):
    ...
    if use_avg and "model_avg" in state_dict:
