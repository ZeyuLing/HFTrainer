...
Wraps the hftrainer-native MDM network + Gaussian diffusion implementation (see
``hftrainer.models.motion.mdm.network``) behind a clean ``ModelBundle``
...
# Repo root: hftrainer/models/motion/mdm/bundle.py -> parents[4]
...
# The MDM config fields we persist into the self-contained hftrainer artifact.
...
class MDMBundle(ModelBundle):
    ...
    def __init__(
        ...
        * **Self-contained hftrainer artifact** — pass ``config`` (the MDM
        ...
        # persistent=True so the stats travel inside any hftrainer checkpoint that
    ...
    def save_pretrained(self, save_directory: str, safe_serialization: bool = True, **kwargs):
        """Export a self-contained hftrainer MDM artifact.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        """Load a self-contained hftrainer MDM artifact (see :meth:`save_pretrained`).

        ``pretrained_model_name_or_path`` may be a local directory **or** a
        HuggingFace Hub repo id (e.g. ``"ZeyuLing/hftrainer-mdm-humanml3d"``),
        which is fetched via ``snapshot_download``.
        """
        ...
        if not cfg_file.exists():
            ...
            # if the artifact is not an hftrainer MDM directory).
