...
_VENDOR_PREFIX = "hftrainer.models.motion.hymotion_v2m.vendor.hymotion"
# hf_trainer repository root (…/hf_trainer).
...
def _rewrite_module_path(path: str) -> str:
    ...
    code lives under ``hftrainer...vendor.hymotion`` (there is no top-level
    ``hymotion`` package inside hf_trainer), rewrite the leading namespace.
    """
    norm = path.replace("/", ".")
...
class HyMotionV2MBundle(ModelBundle):
    ...
    def generate_from_feature(
        ...
        Args:
            ...
            length: number of valid frames in the window (<= train_frames).
