...
class SOMAToSMPLIKConfig:
    ...
    model_dir: str | Path = "ref_repo/MDM/body_models"
...
def _resolve_assets_root(assets_root: str | Path | None) -> Path:
    if assets_root is None:
        ...
        assets_root = env if env else _repo_root() / "ref_repo/KIMODO/kimodo/kimodo/assets/skeletons"
...
def _import_smplx():
    ...
    try:
        ...
        motionstreamer = _repo_root() / "ref_repo/MotionStreamer/272-dim-Motion-Representation"
        vendored = motionstreamer / "utils/smplx"
        for path in (motionstreamer, vendored):
...
def _resolve_smpl_model_dir(model_dir: str | Path) -> Path:
    path = Path(model_dir)
