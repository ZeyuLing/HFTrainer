...
The retargeted G1 AMP npz files (``data/g1/<rel>.npz``) store the full robot
...
generated 38-d motion plugs straight into the same ``MujocoQposConverter`` /
``.motion`` export path used by the PhysFlow physics judge -- no SMPL->G1
retargeting in the online loop.

Conventions match ``hymotion_m2m/network/geometry.py``:
  * AMP ``body_rotations`` are stored as quaternion (x, y, z, w) -- xyzw.
  * ``quaternion_to_rotation_matrix`` expects (w, x, y, z) -- wxyz.
...
def encode_g1_motion(
    ...
    rmat = quaternion_to_rotation_matrix(quat_wxyz)  # (T, 3, 3)
    if canonicalize:
        transl, rmat = _canonicalize_root(transl, rmat)
    rot6d = rotation_matrix_to_rot6d(rmat)  # (T, 6)
...
def decode_g1_to_qpos(motion: torch.Tensor, root_velocity: bool = True) -> torch.Tensor:
    ...
    if root_velocity:
        ...
        transl = transl_feat
    rmat = rot6d_to_rotation_matrix(rot6d)  # (..., 3, 3)
