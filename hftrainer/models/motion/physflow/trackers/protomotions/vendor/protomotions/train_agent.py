...
def create_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Skip the automatic evaluator pass immediately after loading a warm-start checkpoint.",
    )
...
def main():
    ...
    # Auto-detect MuJoCo simulator → force CPU accelerator.
    # PHYSFLOW_FORCE_CPU=1 also forces CPU (for nodes whose GPU driver is too old
    # for IsaacGym GPU PhysX); keeps Fabric device == sim device == cpu.
    _force_cpu = os.environ.get("PHYSFLOW_FORCE_CPU", "0") == "1"
    _accelerator = "cpu" if (args.simulator == "mujoco" or _force_cpu) else "gpu"
    _devices = 1 if (args.simulator == "mujoco" or _force_cpu) else args.ngpu
...
def _handle_create_config_only(
    ...
    _accelerator = "cpu" if args.simulator == "mujoco" else "gpu"
    _devices = 1 if args.simulator == "mujoco" else args.ngpu
    fabric_config = FabricConfig(
        accelerator=_accelerator,
        devices=_devices,
        num_nodes=args.nodes,
