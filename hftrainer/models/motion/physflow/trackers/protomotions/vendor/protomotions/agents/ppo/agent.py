...
def _strip_actor_prefix(name: str) -> str:
    prefix = "_actor."
...
class PPO(BaseAgent):
    ...
    def __init__(
        ...
        self._policy_anchor_params = None
        self._policy_anchor_buffers = None
        self._action_distill_actor = None
    ...
    def load_parameters(self, state_dict):
        ...
        if state_dict.get("skip_optimizer_load", False):
            log.info("Skipping optimizer state load for partial warm-start checkpoint.")
        else:
        ...
        self._setup_policy_anchor()
        self._setup_action_distill()

    def _load_actor_reference_state(self, checkpoint_path: str):
        if not checkpoint_path:
            return None
        ckpt = torch.load(checkpoint_path, map_location=self.device, weights_only=False)
        model_state = ckpt.get("model", ckpt)
        return {
            _strip_actor_prefix(k): v
            for k, v in model_state.items()
            if k.startswith("_actor.")
        }

    def _setup_policy_anchor(self):
        policy_anchor = getattr(self.config, "policy_anchor", None)
        if policy_anchor is None or not policy_anchor.enabled:
            self._policy_anchor_params = None
            self._policy_anchor_buffers = None
            return

        ref_state = None
        ref_path = policy_anchor.reference_checkpoint
        if ref_path:
            ref_state = self._load_actor_reference_state(ref_path)
        ...
        for name, param in self.actor.named_parameters():
            if ref_state is None:
                ref = param.detach()
            elif name in ref_state:
                ref = ref_state[name].to(device=param.device, dtype=param.dtype)
            else:
                missing.append(name)
                continue
            anchor[name] = ref.detach().clone()

        if policy_anchor.freeze_buffers:
            for name, buf in self.actor.named_buffers():
                if ref_state is None:
                    ref = buf.detach()
                elif name in ref_state:
                    ref = ref_state[name].to(device=buf.device, dtype=buf.dtype)
                else:
                    continue
                anchor_buffers[name] = ref.detach().clone()

        self._policy_anchor_params = anchor
        self._policy_anchor_buffers = anchor_buffers
        self._restore_policy_anchor_buffers()
        if self.fabric.global_rank == 0:
            log.info(
                "Policy anchor enabled: coeff=%s reference=%s params=%d buffers=%d missing=%d",
                policy_anchor.coeff,
                ref_path or "<warm-start>",
                len(anchor),
                len(anchor_buffers),
                len(missing),
            )
            if missing:
                log.warning("Policy anchor missing actor params: %s", missing[:10])

    def _setup_action_distill(self):
        action_distill = getattr(self.config, "action_distill", None)
        if (
            action_distill is None
            or not action_distill.enabled
            or action_distill.coeff <= 0
        ):
            self._action_distill_actor = None
            return

        ref_actor = copy.deepcopy(self.actor)
        ref_path = action_distill.reference_checkpoint
        missing = []
        if ref_path:
            ref_state = self._load_actor_reference_state(ref_path)
            own_state = ref_actor.state_dict()
            load_state = {}
            for name, value in own_state.items():
                ref = ref_state.get(name) if ref_state is not None else None
                if ref is None:
                    missing.append(name)
                    load_state[name] = value
                else:
                    load_state[name] = ref.to(device=value.device, dtype=value.dtype)
            ref_actor.load_state_dict(load_state, strict=False)

        ref_actor.eval()
        for param in ref_actor.parameters():
            param.requires_grad_(False)
        self._action_distill_actor = ref_actor

        if self.fabric.global_rank == 0:
            log.info(
                "Action distill enabled: coeff=%s reference=%s missing=%d",
                action_distill.coeff,
                ref_path or "<warm-start>",
                len(missing),
            )
            if missing:
                log.warning("Action distill missing actor state: %s", missing[:10])

    @torch.no_grad()
    def _restore_policy_anchor_buffers(self):
        if not self._policy_anchor_buffers:
            return
        for name, buf in self.actor.named_buffers():
            ref = self._policy_anchor_buffers.get(name)
            if ref is not None:
                buf.copy_(ref.to(device=buf.device, dtype=buf.dtype))
    ...
    def calculate_extra_actor_loss(self, batch_td) -> Tuple[Tensor, Dict]:
        ...
        # --- Conservative actor anchor ---
        policy_anchor = getattr(self.config, "policy_anchor", None)
        if (
            policy_anchor is not None
            and policy_anchor.enabled
            and policy_anchor.coeff > 0
            and self._policy_anchor_params
        ):
            anchor_ss = torch.tensor(0.0, device=self.device)
            anchor_n = 0
            for name, param in self.actor.named_parameters():
                ref = self._policy_anchor_params.get(name)
                if ref is None:
                    continue
                anchor_ss = anchor_ss + (param - ref).pow(2).sum()
                anchor_n += param.numel()
            if anchor_n > 0:
                anchor_loss = anchor_ss / anchor_n
                anchor_weighted = policy_anchor.coeff * anchor_loss
                extra_loss = extra_loss + anchor_weighted
                log_dict.update(
                    {
                        "actor/policy_anchor_loss": anchor_loss.detach(),
                        "actor/policy_anchor_weighted": anchor_weighted.detach(),
                    }
                )

        # --- Behavior-level action distillation ---
        if self._action_distill_actor is not None:
            ref_td = TensorDict(
                {
                    key: batch_td[key].detach()
                    for key in self.actor.in_keys
                    if key in batch_td
                },
                batch_size=batch_td["mean_action"].shape[0],
            )
            with torch.no_grad():
                ref_td = self._action_distill_actor(ref_td)
                ref_mu = ref_td["mean_action"]
            action_distill_loss = (batch_td["mean_action"] - ref_mu).pow(2).mean()
            action_distill_weighted = (
                self.config.action_distill.coeff * action_distill_loss
            )
            extra_loss = extra_loss + action_distill_weighted
            log_dict.update(
                {
                    "actor/action_distill_loss": action_distill_loss.detach(),
                    "actor/action_distill_weighted": action_distill_weighted.detach(),
                }
            )
