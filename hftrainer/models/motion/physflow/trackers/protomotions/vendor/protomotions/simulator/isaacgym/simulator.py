...
class IsaacGymSimulator(Simulator):
    ...
    def __init__(
        ...
        # --headless arrives from argparse as the *string* "True"/"False", so an
        # `is True` identity check silently fails and leaves graphics enabled.
        _headless = self.headless
        if isinstance(_headless, str):
            _headless = _headless.strip().lower() in ("1", "true", "yes", "on")
        self._graphics_device_id = device_index
        if _headless:
            # Headless training never renders (no viewer / cameras), so disable
            # the graphics pipeline entirely. A real graphics device makes
            # create_sim build a Vulkan context that segfaults on a display-less
            # Taiji node; -1 == physics-only (verified to create_sim cleanly).
