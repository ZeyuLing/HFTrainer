...
class PolicyAnchorConfig:
    """Actor parameter anchor for conservative fine-tuning.

    Keeps the policy near a reference checkpoint while still allowing PPO to
    adapt to new hard motions. This is intentionally actor-only: critic and
    discriminator can relearn values/style for the new pool without directly
    moving the deployment policy.
    """

    enabled: bool = field(default=False, metadata={"help": "Enable actor policy anchor."})
    coeff: float = field(default=0.0, metadata={"help": "Mean-squared actor parameter anchor coefficient."})
    reference_checkpoint: str = field(
        default="",
        metadata={"help": "Checkpoint whose actor parameters are used as the anchor. Empty means current warm-start."},
    )
    freeze_buffers: bool = field(
        default=True,
        metadata={"help": "Restore actor buffers such as observation normalizers to the anchor after updates."},
    )


@dataclass
class ActionDistillConfig:
    """Behavior-level actor distillation for conservative fine-tuning.

    Penalizes the current actor mean action from drifting away from a reference
    actor on the same PPO minibatch observations. This is softer than parameter
    anchoring because it preserves deployment behavior directly while still
    allowing different parameterizations where the behavior is unchanged.
    """

    enabled: bool = field(default=False, metadata={"help": "Enable action distillation."})
    coeff: float = field(default=0.0, metadata={"help": "Mean-squared mean-action distillation coefficient."})
    reference_checkpoint: str = field(
        default="",
        metadata={"help": "Checkpoint whose actor outputs are used as the reference. Empty means current warm-start."},
    )
...
class PPOAgentConfig(BaseAgentConfig):
    ...
    # Conservative fine-tuning anchor
    policy_anchor: PolicyAnchorConfig = field(
        default_factory=PolicyAnchorConfig, metadata={"help": "Policy anchor settings."}
    )

    # Conservative behavior-level distillation
    action_distill: ActionDistillConfig = field(
        default_factory=ActionDistillConfig, metadata={"help": "Action distillation settings."}
    )
