...
class MimicEvaluator(BaseEvaluator):
    ...
    def run_evaluation(self) -> None:
        ...
        eval_batches = self._build_eval_batches()
        for batch_idx, (env_ids, motion_ids) in enumerate(eval_batches):
            ...
            print(
                "Running eval batch "
                f"{batch_idx + 1}/{len(eval_batches)}: "
                f"motions {motion_ids[0].item()} to {motion_ids[-1].item() + 1}, "
                f"max_steps={max_len}",
                flush=True,
            )
            ...
            print(
                f"Finished eval batch {batch_idx + 1}/{len(eval_batches)}",
                flush=True,
            )
    ...
    def _build_eval_batches(self):
        ...
        for start in range(0, num_motions, self.num_envs):
            ...
            print(
                f"Evaluating motions {start} to {end}, out of total {num_motions}",
                flush=True,
            )
    ...
    def process_eval_results(self) -> Tuple[Dict, Optional[float]]:
        ...
        if self.fabric.global_rank == 0:
            if (
                ...
                print("Saving predicted MotionLib...", flush=True)
    ...
    def _save_predicted_motion_lib(
        ...
        print(f"Predicted MotionLib saved to {output_path}", flush=True)
