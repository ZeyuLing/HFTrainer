...
class MujocoSimulator(Simulator):
    ...
    def __init__(
        ...
        # Body COM offsets for velocity correction (MuJoCo velocity semantics)
        # MuJoCo's data.cvel returns velocity at frame origin, not COM.
        # We extract COM offsets to apply correction: v_COM = v_frame + ω × r_offset
        # This ensures MuJoCo observations match IsaacGym (COM velocity) semantics.
        self._body_com_offsets = None  # [num_robot_bodies, 3] in local body frame
        # Body/DOF indexing
    ...
    def _create_simulation(self) -> None:
        ...
        # Must be set before _extract_body_com_offsets() which uses it
    ...
    def _configure_actuators_for_pd(self) -> None:
        ...
        for act_idx in range(self.model.nu):
            ...
            # force = gear * (gainprm[0] * ctrl + biasprm[0] + biasprm[1] * q + biasprm[2] * qd)
            #       = 1.0 * (kp * ctrl + 0 + (-kp) * q + (-kd) * qd)
            #       = kp * (ctrl - q) - kd * qd
            #
            # CRITICAL: Reset gear to 1.0! The MJCF may have gear=500 for motor
            # actuators. Without resetting, force is multiplied by 500×.
            self.model.actuator_gear[act_idx, 0] = 1.0
    ...
    def _configure_explicit_pd(self) -> None:
        ...
        IMPORTANT: Reset gear to 1.0 so ctrl values are interpreted as direct
        torques (not multiplied by gear=500 from original motor actuators).
        """
        control_info = self.robot_config.control.control_info
        ...
        for act_idx in range(self.model.nu):
            jnt_id = self.model.actuator_trnid[act_idx, 0]
            jnt_name = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, jnt_id)
            # Reset gear to 1.0 so ctrl = direct torque
            self.model.actuator_gear[act_idx, 0] = 1.0
            if jnt_name in control_info:
    ...
    def _quat_rotate_inverse_np(q_xyzw: np.ndarray, v: np.ndarray) -> np.ndarray:
        """Convert vector from world frame to body frame via quaternion rotation.

        Implements the inverse rotation: expresses the world-frame vector ``v``
        in the body frame defined by quaternion ``q``.

        Formula (matches TorchScript implementation in rotations.py):
            a = v * (2 * q_w^2 - 1)
    ...
    def _set_simulator_env_state(
        ...
        if self._has_free_joint:
            ...
            # but root_ang_vel from RobotState is in WORLD frame (from data.cvel).
    ...
    def _extract_body_com_offsets(self) -> None:
        """Extract center-of-mass (COM) offsets for each body from MuJoCo model.

        MuJoCo body COM is computed from all geometries in the body. We extract
        these offsets relative to the body frame origin. This is essential for
        correcting MuJoCo velocity semantics to match IsaacGym.
        ...
        **Velocity Semantics Note:**
        ...
        The extracted offsets are stored in self._body_com_offsets [num_robot_bodies, 3]
        in the local body frame.
        """
        nb = self._num_robot_bodies
        body_com_offsets = np.zeros((nb, 3), dtype=np.float32)

        # MuJoCo bodies are indexed 1..nbody (0 is world)
        for body_idx in range(1, 1 + nb):
            ...
            # (assumes roughly uniform mass distribution; could be refined
            # by parsing MJCF geom densities, but this is a good approximation)
        ...
        offsets_magnitude = np.linalg.norm(body_com_offsets, axis=1)
        nonzero_count = np.sum(offsets_magnitude > 1e-6)
        max_magnitude = np.max(offsets_magnitude)
        max_body_idx = np.argmax(offsets_magnitude)

        log.info(
            "COM offsets extracted: %d/%d bodies have non-zero offsets, "
            "max offset: %.4f m (body %d)",
            nonzero_count,
            nb,
            max_magnitude,
            max_body_idx + 1,
    ...
    def _apply_com_velocity_correction(
        self,
        body_vel_frame: np.ndarray,
        body_ang_vel: np.ndarray,
        body_rot: np.ndarray,
    ) -> np.ndarray:
        """Apply COM offset correction to body velocities.

        Converts frame-origin velocities (from MuJoCo data.cvel) to COM velocities
        to match IsaacGym physics engine semantics.

        The correction formula is:
            v_COM = v_frame_origin + ω × r_offset_world

        where:
            - v_frame_origin: velocity at body frame origin (from data.cvel)
            - ω: angular velocity in world frame (from data.cvel)
            - r_offset_world: COM offset in world frame (rotated from local)

        Args:
            body_vel_frame: [nb, 3] velocities at frame origin (from data.cvel)
            body_ang_vel: [nb, 3] angular velocities (from data.cvel)
            body_rot: [nb, 4] body quaternions in wxyz format

        Returns:
            [nb, 3] corrected velocities at COM frame
        """
        nb = body_vel_frame.shape[0]
        body_vel_com = body_vel_frame.copy()

        # For each body, rotate COM offset to world frame and apply correction
        for i in range(nb):
            # Get COM offset in local frame
            ...
            # body_rot[i] is in wxyz format
            quat_wxyz = body_rot[i]

            # Quaternion rotation: v_world = R(q) * v_local
            # Using formula: v' = q * (v, 0) * q^*
            w, x, y, z = quat_wxyz[0], quat_wxyz[1], quat_wxyz[2], quat_wxyz[3]
            vx, vy, vz = com_offset_local[0], com_offset_local[1], com_offset_local[2]

            # q * v * q^* expanded (optimized quaternion rotation)
            # Compute q * v (quaternion-vector multiplication)
            qv_w = -x * vx - y * vy - z * vz
            qv_x = w * vx + y * vz - z * vy
            qv_y = w * vy + z * vx - x * vz
            qv_z = w * vz + x * vy - y * vx

            # Multiply by q^* (conjugate)
            com_offset_world = np.array([
                qv_x * w + qv_w * (-x) + qv_y * (-z) - qv_z * (-y),
                qv_y * w + qv_w * (-y) + qv_z * (-x) - qv_x * (-z),
                qv_z * w + qv_w * (-z) + qv_x * (-y) - qv_y * (-x),
            ], dtype=np.float32)
    ...
    def _get_simulator_bodies_state(
        ...
        """Get rigid body states from MuJoCo FK results with COM velocity correction.
        
        **Velocity Semantics Note:**
        MuJoCo data.cvel returns velocity at the body frame origin, but IsaacGym
        returns velocity at the center-of-mass (COM). This method applies a
        correction to convert frame-origin velocities to COM velocities using:
            v_COM = v_frame_origin + ω × r_offset_world
        
        This ensures that velocities returned by MuJoCo match IsaacGym semantics
        for consistent downstream processing.
        
        Returns:
            RobotState: Rigid body states with COM-corrected velocities
        """
        nb = self._num_robot_bodies
        ...
        body_vel = self._apply_com_velocity_correction(body_vel_frame, body_ang_vel, body_rot)
