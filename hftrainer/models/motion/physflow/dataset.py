"""PhysFlow prompt dataset: yields a text prompt + its pre-extracted KIMODO
text embedding (no 8B encoder at train time) + the target motion length.

(Lives in the physflow package because ``hftrainer/datasets/`` is cursorignored;
registered into the global DATASETS registry all the same.)

The embedding cache is produced by
``scripts/embodied/cursor_extract_kimodo_text_feature.py`` and stored as
``<feature_dir>/<sha256>.npy`` ([seq, 4096]) with a ``manifest.jsonl`` mapping
prompt/id -> key. Durations come from the prompt corpus jsonl.
"""
