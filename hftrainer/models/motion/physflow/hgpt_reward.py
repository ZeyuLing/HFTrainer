...
py3.11 env that cannot be imported into the KIMODO training process. So we talk to
...
from hftrainer.models.motion.physflow.trackers.paths import HUMANOID_GPT_ONNX, HUMANOID_GPT_ROOT
...
class HgptJudgeReward:
    def __init__(
        ...
        **kwargs,
        ...
        self.hgpt_root = Path(hgpt_root or HUMANOID_GPT_ROOT).resolve()
        ...
        # Env var wins so the judge venv can be relocated (e.g. fast ephemeral
        # /root) without editing configs; then the config value; then the
        # in-repo persistent venv default.
        _hgpt_python = Path(
            os.environ.get("PHYSFLOW_HGPT_PYTHON")
            or hgpt_python
            or str(self.hgpt_root / ".venv311" / "bin" / "python")
        )
    ...
    def _ensure_worker(self) -> None:
        ...
        if not os.path.exists(self.hgpt_python):
            raise FileNotFoundError(
                ...
                f"(set PHYSFLOW_HGPT_PYTHON or build {self.hgpt_root}/.venv311)"
            )
        cmd = [
            self.hgpt_python,
        ...
        # These Taiji V100 nodes may not expose EGL correctly inside the
        # py3.11 HGPT worker. The judge never renders, so disable GL eagerly.
        env.setdefault("MUJOCO_GL", "disable")
        log_path = self.hgpt_root / "physflow_hgpt_judge_server.log"
