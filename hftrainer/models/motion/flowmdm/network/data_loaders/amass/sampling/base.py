...
import numpy as np
# from hftrainer.models.motion.flowmdm.network.utils.parser_util import frame_sampler_parser
...
class FrameSampler:
    ...
    def can_be_cropped(self, duration):
        return duration >= self.min_len
    
    def sample(self, duration):
        if duration < self.min_len:
            raise ValueError(f"duration [{duration}] is less than min_len [{self.min_len}]")
        elif duration == self.min_len:
            return 0, duration
        
        # sample a random subset of frames that satisfies the sampler constrains: >= min and <= max
        random_length = np.random.randint(self.min_len, self.max_len) if self.max_len > self.min_len else self.min_len
        random_start = np.random.randint(0, duration - random_length)
        return random_start, random_start + random_length
