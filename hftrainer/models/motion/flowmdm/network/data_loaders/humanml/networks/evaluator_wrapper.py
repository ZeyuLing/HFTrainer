...
from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.utils.word_vectorizer import POS_enumerator
...
def build_evaluators(opt):
    ...
    if opt['dataset_name'] == 'humanml':
        ckpt_dir = 't2m/humanml'
    elif opt['dataset_name'] == 'babel':
        ckpt_dir = 't2m/babel'

    model_name = 'text_mot_match'

    checkpoint = torch.load(pjoin(opt['checkpoints_dir'], ckpt_dir, model_name, 'model', 'finest.tar'), map_location=opt['device']) # original code
