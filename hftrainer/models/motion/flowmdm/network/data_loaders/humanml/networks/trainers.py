...
import random
from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.networks.modules import *
from torch.utils.data import DataLoader
import torch.optim as optim
from torch.nn.utils import clip_grad_norm_
# import tensorflow as tf
from collections import OrderedDict
from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.utils.utils import *
from os.path import join as pjoin
from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.data.dataset import collate_fn
import codecs as cs
from hftrainer.models.motion.flowmdm.network.utils.train_platforms import ClearmlPlatform, TensorboardPlatform, NoPlatform  # required for the eval operation
...
class Logger2(object):
    def __init__(self, args):
        self.train_platform_type = eval(args.train_platform_type)
        self.train_platform = self.train_platform_type(args.log_dir)
        self.train_platform.report_args(args, name='Args')

    def scalar_summary(self, tag, value, step):
        self.train_platform.report_scalar(name=tag, value=value, iteration=step, group_name='Loss')
...
class LoggerToFile(object):
    ...
    def scalar_summary(self, tag, value, step):
        with cs.open(pjoin(self.log_dir, 'log.txt'), 'a', 'utf-8') as f:
            f.write('%s: %.5f\n' % (tag, value))
...
class DecompTrainerV3(object):
    ...
    def forward(self, batch_data):
        ...
        if self.opt.foot_contact_entries > 0:
            _motions = self.motions[..., :-self.opt.foot_contact_entries]
        else:
            _motions = self.motions
        self.latents = self.movement_enc(_motions)
...
class CompTrainerV6(object):
    ...
    def forward(self, batch_data, tf_ratio, mov_len, eval_mode=False):
        ...
        if self.opt.foot_contact_entries > 0:
            ...
            _motions = self.motions
        self.movements = self.mov_enc(_motions).detach()
        # Initially input a mean vector
        mov_in = self.mov_enc(
            torch.zeros((self.motions.shape[0], self.opt.unit_length, self.motions.shape[-1] - self.opt.foot_contact_entries), device=self.device)
        ).squeeze(1).detach()
    ...
    def generate(self, word_emb, pos_ohot, cap_lens, m_lens, mov_len, dim_pose):
        ...
        # Initially input a mean vector
        mov_in = self.mov_enc(
            torch.zeros((word_emb.shape[0], self.opt.unit_length, dim_pose - self.opt.foot_contact_entries), device=self.device)
        ).squeeze(1).detach()
...
class TextMotionMatchTrainer(object):
    ...
    def __init__(self, args, text_encoder, motion_encoder, movement_encoder):
        ...
        if args.is_train:
            # self.motion_dis
            self.logger = LoggerToFile(args.log_dir)
    ...
    def forward(self, batch_data):
        word_emb, pos_ohot, caption, cap_lens, motions, m_lens = batch_data[:6]
        word_emb = word_emb.detach().to(self.device).float()
        pos_ohot = pos_ohot.detach().to(self.device).float()
        motions = motions.detach().to(self.device).float()
    ...
    def compute_new_pairs(self, batch_data):
        captions = batch_data[2]
        new_idces = []
        for i in range(len(captions)):
            different_captions = False
            while not different_captions:
                new_idx = np.random.randint(0, len(captions))
                if new_idx != i and captions[i] != captions[new_idx]:
                    different_captions = True    
                elif new_idx != i:
                    pass
            new_idces.append(new_idx)
        return new_idces
    ...
    def backward(self, batch_data):
        ...
        new_idx = self.compute_new_pairs(batch_data)
    ...
    def update(self, batch_data):
        ...
        loss_logs = self.backward(batch_data)
        self.loss.backward()
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_dataloader):
                ...
                self.forward(batch_data)
                # time3 = time.time()
                log_dict = self.update(batch_data)
                for k, v in log_dict.items():
                    if k not in logs:
                        logs[k] = v
                    else:
