...
from hftrainer.models.motion.flowmdm.network.data_loaders.tensors import collate as all_collate
from hftrainer.models.motion.flowmdm.network.data_loaders.tensors import t2m_collate

def get_dataset_class(name, load_mode):
    if name == "babel":
        if 'gt' in load_mode or load_mode == 'evaluator_train': # reference motion for evaluation
            from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.data.dataset import BABEL_eval
            return BABEL_eval
        elif load_mode == 'gen':
            from hftrainer.models.motion.flowmdm.network.data_loaders.amass.babel import BABEL
            return BABEL
        elif load_mode == 'train':
            from hftrainer.models.motion.flowmdm.network.data_loaders.amass.babel_flowmdm import BABEL
            return BABEL
        else:
            raise ValueError(f'Unsupported load_mode name [{load_mode}]')
    elif name == "humanml":
        from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.data.dataset import HumanML3D
        return HumanML3D
    else:
...
def get_collate_fn(name, load_mode='train'):
    ...
    if "gt" in load_mode:
        from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.data.dataset import collate_fn as t2m_eval_collate
        return t2m_eval_collate
    if name in ["humanml"]:
        return t2m_collate
    elif name == 'babel' and load_mode != "evaluator_train":
        from hftrainer.models.motion.flowmdm.network.data_loaders.tensors import babel_collate
        return babel_collate
    elif name == 'babel':
        from hftrainer.models.motion.flowmdm.network.data_loaders.humanml.data.dataset import collate_fn as sorted_collate
...
def get_dataset(name, num_frames, split='train', load_mode='train', **kwargs):
    ...
    if name in ["humanml"]:
        ...
        from hftrainer.models.motion.flowmdm.network.data_loaders.amass.transforms import SlimSMPLTransform
