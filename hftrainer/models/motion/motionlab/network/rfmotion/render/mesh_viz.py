...
from hftrainer.models.motion.motionlab.network.rfmotion.utils.smpl_body_utils import get_smpl_skeleton
...
def render_motion(renderer: HeadlessRenderer, datum: dict, 
                  filename: str, text_for_vid=None, hint=None, hint_masks=None, pose_repr='6d',
    ...
    from aitviewer.renderables.smpl import SMPLSequence
    import trimesh
    from hftrainer.models.motion.motionlab.network.rfmotion.render.video import put_text
    from hftrainer.models.motion.motionlab.network.rfmotion.tools.transforms3d import transform_body_pose
    ...
    for iid, mesh_seq in enumerate(datum):
        ...
        if hint is not None:
            for i in range(hint_masks.shape[0]):
                for j in range(hint_masks.shape[1]):
                    if hint_masks[i, j, 0] == 1:
                        hint_mesh = trimesh.primitives.Sphere(radius=0.01)
                        hint_mesh_ait = Meshes(
                                            hint_mesh.vertices,
                                            hint_mesh.faces,
                                            name="ball_{}_{}".format(i,j),
                                            position=hint[i,j,:],
                                            color=(0.5, 0, 0, 1),
                                            flat_shading=True,
                                        )
                        ball_list.append(hint_mesh_ait)
                        renderer.scene.add(hint_mesh_ait)

        
    # camera follows smpl sequence
    # FIX CAMERA
    from hftrainer.models.motion.motionlab.network.rfmotion.tools.transforms3d import get_z_rot
    ...
    if len(mesh_seq['body_pose']) == 1:
        ...
        renderer.save_video(video_dir=str(filename), output_fps=20)
        sfx = 'mp4'
...
# from hftrainer.models.motion.motionlab.network.rfmotion.utils.mesh_utils import MeshViewer
# from hftrainer.models.motion.motionlab.network.rfmotion.utils.smpl_body_utils import colors,marker2bodypart,bodypart2color
# from hftrainer.models.motion.motionlab.network.rfmotion.utils.smpl_body_utils import c2rgba, get_body_model
# from hydra.utils import get_original_cwd
# from hftrainer.models.motion.motionlab.network.rfmotion.render.video import save_video_samples
...
#     if save_path is not None:
#         from hftrainer.models.motion.motionlab.network.rfmotion.render.video import Video
