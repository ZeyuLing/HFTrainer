...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if self.added_kv_proj_dim is not None:
            self.source_k_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.source_v_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.text_k_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.text_v_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.hint_k_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.hint_v_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.style_k_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.style_v_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.content_k_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            self.content_v_proj = nn.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias)
            if self.context_pre_only is not None:
                self.source_q_proj = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
                self.text_q_proj = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
                self.hint_q_proj = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
                self.style_q_proj = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
                self.content_q_proj = nn.Linear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
        ...
        if self.context_pre_only is not None and not self.context_pre_only:
            self.to_source_out = nn.Linear(self.inner_dim, self.out_context_dim, bias=out_bias)
            self.to_text_out = nn.Linear(self.inner_dim, self.out_context_dim, bias=out_bias)
            self.to_hint_out = nn.Linear(self.inner_dim, self.out_context_dim, bias=out_bias)
            self.to_style_out = nn.Linear(self.inner_dim, self.out_context_dim, bias=out_bias)
            self.to_content_out = nn.Linear(self.inner_dim, self.out_context_dim, bias=out_bias)

        if qk_norm is not None and added_kv_proj_dim is not None:
            if qk_norm == "fp32_layer_norm":
                self.norm_source_q = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_source_k = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_text_q = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_text_k = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_hint_q = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_hint_k = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_style_q = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_style_k = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_content_q = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_content_k = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
            elif qk_norm == "rms_norm":
                self.norm_source_q = RMSNorm(dim_head, eps=eps)
                self.norm_source_k = RMSNorm(dim_head, eps=eps)
                self.norm_text_q = RMSNorm(dim_head, eps=eps)
                self.norm_text_k = RMSNorm(dim_head, eps=eps)
                self.norm_hint_q = RMSNorm(dim_head, eps=eps)
                self.norm_hint_k = RMSNorm(dim_head, eps=eps)
                self.norm_style_q = RMSNorm(dim_head, eps=eps)
                self.norm_style_k = RMSNorm(dim_head, eps=eps)
                self.norm_content_q = RMSNorm(dim_head, eps=eps)
                self.norm_content_k = RMSNorm(dim_head, eps=eps)
            else:
                raise ValueError(
                    f"unknown qk_norm: {qk_norm}. Should be one of `None,'layer_norm','fp32_layer_norm','rms_norm'`"
                )
        else:
            self.norm_source_q = None
            self.norm_source_k = None
            self.norm_text_q = None
            self.norm_text_k = None
            self.norm_hint_q = None
            self.norm_hint_k = None
            self.norm_style_q = None
            self.norm_style_k = None
            self.norm_content_q = None
            self.norm_content_k = None

        self.q_pos_embed = build_position_encoding(dim_head, position_embedding='learned', embedding_dim="1D")
        self.k_pos_embed = build_position_encoding(dim_head, position_embedding='learned', embedding_dim="1D")
        self.style_q_pos_embed = build_position_encoding(dim_head, position_embedding='learned', embedding_dim="1D")
        self.style_k_pos_embed = build_position_encoding(dim_head, position_embedding='learned', embedding_dim="1D")
        self.content_q_pos_embed = build_position_encoding(dim_head, position_embedding='learned', embedding_dim="1D")
        self.content_k_pos_embed = build_position_encoding(dim_head, position_embedding='learned', embedding_dim="1D")
        self.ROPE = RotaryEmbedding(dim_head, use_xpos = True)
...
class AllegroAttnProcessor2_0:
    ...
    def __call__(
        ...
        if image_rotary_emb is not None and not attn.is_cross_attention:
            from diffusers.embeddings import apply_rotary_emb_allegro
...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if image_rotary_emb is not None:
            from diffusers.embeddings import apply_rotary_emb
