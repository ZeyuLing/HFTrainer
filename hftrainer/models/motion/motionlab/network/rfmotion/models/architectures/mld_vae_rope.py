...
class MldVae(nn.Module):
    ...
    def decode(self, z: Tensor, lengths: List[int], lengths_z: Optional[List[int]] = None):
        x_mask = lengths_to_mask(lengths, z.device, max_len=z.shape[0])
        z_index, z_mask = lengths_to_query_mask(lengths, lengths_z, z.device, max_len=z.shape[0]) 
        nframes, bs, _ = z.shape

        # input tokens
        z = self.decode_embedding(z.permute(1, 0, 2))
        x_query = torch.tile(self.x_query[None, :, :], (bs, nframes, 1))
        ...
        # decode xseq
        output = self.decoder(xseq, src_key_padding_mask=~augmask)[nframes:]
