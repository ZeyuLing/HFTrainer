...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    if attn_mask is not None:
        if attn_mask.dtype == torch.bool:
            attn_bias.masked_fill_(attn_mask.logical_not(), float("-1e20"))
        else:
            attn_bias += attn_mask
...
class JointAttnProcessor2_0:
    ...
    def __init__(self):
        ...
        self.pos_embed1 = build_position_encoding(int(512/8), position_embedding="learned", embedding_dim="1D")
        self.pos_embed2 = build_position_encoding(int(512/8), position_embedding="learned", embedding_dim="1D")
        self.pos_embed3 = build_position_encoding(int(512/8), position_embedding="learned", embedding_dim="1D")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if attention_type == "self":
            ...
            # `sample` projections.
            self.pos_embed1 = self.pos_embed1.to(hidden_states.device)
            self.pos_embed2 = self.pos_embed2.to(hidden_states.device)
            self.pos_embed3 = self.pos_embed3.to(hidden_states.device)
            ...
            if attn.norm_q is not None:
                query = attn.norm_q(query)
            query = self.pos_embed1(query, batch_first=True) + query

            # `source_motion` projections.
            if source_motion is not None:
                source_motion_key_proj = attn.add_k_proj(source_motion)
                source_motion_value_proj = attn.add_v_proj(source_motion)
                ...
                if attn.norm_added_k is not None:
                    source_motion_key_proj = attn.norm_added_k(source_motion_key_proj)

                source_motion_key_proj = self.pos_embed2(source_motion_key_proj, batch_first=True) + source_motion_key_proj
                key = source_motion_key_proj
                value = source_motion_value_proj
            ...
            if encoder_hidden_states is not None:
                ...
                encoder_hidden_states_key_proj = self.pos_embed3(encoder_hidden_states_key_proj, batch_first=True) + encoder_hidden_states_key_proj
            ...
            if source_motion is not None:
                if not attn.context_pre_only:
                    source_motion = attn.to_add_out(source_motion_value_proj.reshape(batch_size, -1, attn.heads * head_dim))

            if encoder_hidden_states is None:
                if not attn.context_pre_only:
                    encoder_hidden_states = attn.to_addd_out(encoder_hidden_states_value_proj.reshape(batch_size, -1, attn.heads * head_dim))
...
class JointTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm2 = AdaLayerNormZero(dim)
    ...
    def forward(
        ...
        norm1_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.norm1(hidden_states, emb=temb)
        ...
        self_attention_mask = self_attention_mask.unsqueeze(1).unsqueeze(2).repeat(1, self.self_attn.heads, target_mask.size(1), 1)
        self_attn_output = self.self_attn(hidden_states=norm1_hidden_states, attention_mask=self_attention_mask, attention_type="self")
        self_attn_output = gate_msa.unsqueeze(1) * self_attn_output
        self_attn_output = hidden_states + self_attn_output

        # Cross Attention.
        norm2_hidden_states, gate_mca, _, _, _ = self.norm2(self_attn_output, emb=temb)
        cross_attention_mask = torch.cat([source_mask, text_mask], dim=-1)
        cross_attention_mask = compute_boolean_matrix(target_mask, cross_attention_mask).unsqueeze(1).repeat(1, self.cross_attn.heads, 1, 1)
        cross_attn_output, source_attn_output, context_attn_output = self.cross_attn(hidden_states=norm2_hidden_states, source_motion=norm_source_motion, encoder_hidden_states=norm_encoder_hidden_states, attention_mask=cross_attention_mask, attention_type="cross")
        cross_attn_output = gate_mca.unsqueeze(1) * cross_attn_output
        cross_attn_output = hidden_states + cross_attn_output

        # FFN for the `hidden_states`.
        norm3_hidden_states = self.norm3(cross_attn_output)
        norm3_hidden_states = norm3_hidden_states * (1 + scale_mlp[:, None]) + shift_mlp[:, None]
        if self._chunk_size is not None:
            # "feed_forward_chunk_size" can be used to save memory
            ff_output = _chunked_feed_forward(self.ff, norm3_hidden_states, self._chunk_dim, self._chunk_size)
        else:
            ff_output = self.ff(norm3_hidden_states)
        ff_output = gate_mlp.unsqueeze(1) * ff_output
        hidden_states = hidden_states + ff_output

        # FFN for the `encoder_hidden_states`.
        if self.context_pre_only:
