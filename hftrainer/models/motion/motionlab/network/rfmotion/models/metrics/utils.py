...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    if not torch.isfinite(covmean).all():
        ...
        covmean = torch.sqrt(torch.mm(sigma1+ offset,sigma2+ offset))
...
def calculate_diversity(activation, diversity_times):
    ...
    first_indices = torch.random.choice(num_samples,
                                     diversity_times,
                                     replace=False)
    second_indices = torch.random.choice(num_samples,
                                      diversity_times,
                                      replace=False)
    dist = torch.norm(activation[first_indices] - activation[second_indices],
                       axis=1)
...
def calc_pampjpe(preds, target, sample_wise=True, return_transform_mat=False):
    ...
    preds_tranformed, PA_transform = batch_compute_similarity_transform_torch(preds, target)
