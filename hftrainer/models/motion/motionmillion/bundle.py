...
Wraps the hftrainer-native ICCV'25 "Go to Zero: Towards Zero-shot Motion
Generation with Million-scale Data" (MotionMillion) text-to-motion
implementation behind a clean ``ModelBundle`` interface.

Architecture (implemented in ``hftrainer.models.motion.motionmillion.network``):
...
* **Flan-T5-XL** text encoder (frozen, reloaded by name; not stored in the
  hftrainer artifact, exactly like CLIP in MDM / SentenceT5 in MotionStreamer).
...
# Repo root: hftrainer/models/motion/motionmillion/bundle.py -> parents[4]
...
def _tolerant_pickle_module():
    ...
    shim = types.ModuleType("hftrainer_tolerant_pickle_motionmillion")
...
class MotionMillionBundle(ModelBundle):
        ...
        * **Explicit raw upstream checkpoints** — ``fsq_path`` (``.zip``/``.pth``
        ...
        if fsq_weights_path is None and fsq_path is None:
            raise ValueError(
                ...
                "hftrainer artifact or an explicit raw fsq_path for conversion."
            )
        if ar_weights_path is None and ar_path is None:
            raise ValueError(
                "MotionMillionBundle requires ar_weights_path from an "
                "hftrainer artifact or an explicit raw ar_path for conversion."
            )
    ...
    def save_pretrained(self, save_directory: str, safe_serialization: bool = True, **kwargs):
        """Export a self-contained hftrainer MotionMillion artifact.
        ...
        if not readme.exists():
            readme.write_text(
                ...
                "Load with `MotionMillionPipeline.from_pretrained(...)` from hftrainer.\n",
            )
        return save_directory

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        """Load a self-contained hftrainer MotionMillion artifact (local dir or HF Hub id)."""
