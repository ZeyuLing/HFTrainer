...
Wraps the hftrainer-native MotionStreamer implementation (causal TAE + LLaMA
autoregressive transformer + per-token diffusion head; see
``hftrainer.models.motion.motionstreamer.network``) behind a clean
``ModelBundle`` interface.

The text encoder is SentenceT5-XXL (loaded by name via ``sentence_transformers``,
frozen, and -- like CLIP in MDM -- not duplicated into the hftrainer artifact).
...
# Repo root: hftrainer/models/motion/motionstreamer/bundle.py -> parents[4]
...
# travel inside the hftrainer artifact so a checkpoint is self-contained.
...
class MotionStreamerBundle(ModelBundle):
    ...
    def __init__(
        ...
        * **Self-contained hftrainer artifact** — pass ``config`` plus
        ...
        if tae_weights_path is None and tae_path is None:
            raise ValueError(
                ...
                "hftrainer artifact or an explicit raw tae_path for conversion."
            )
    ...
    def save_pretrained(self, save_directory: str, safe_serialization: bool = True, **kwargs):
        """Export a self-contained hftrainer MotionStreamer artifact.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        """Load a self-contained hftrainer MotionStreamer artifact.
