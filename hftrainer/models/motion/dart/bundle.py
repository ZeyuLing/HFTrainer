...
hftrainer model-zoo style. Runtime imports come from the vendored
``hftrainer.models.motion.dart.network`` tree and never from ``ref_repo``.
"""
...
_DEFAULT_ARTIFACT = _REPO_ROOT / "checkpoints" / "dart" / "hftrainer_hml3d"
...
class DARTBundle(ModelBundle):
    ...
    def _motion_sequence_to_motion135(self, sequence: dict) -> np.ndarray:
        ...
        if self.coord_conversion == "mbench":
            from hftrainer.motion.representation.dart276 import MBENCH_COORD_CONVERSION

            conv = MBENCH_COORD_CONVERSION.to(device=global_orient.device, dtype=global_orient.dtype)
        ...
        rot6d = matrix_to_rotation_6d(rot.float(), convention="row").reshape(rot.shape[0], 132)
