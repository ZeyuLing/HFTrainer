...
from hftrainer.models.motion.kimodo.network.motion_rep.feature_utils import compute_heading_angle, length_to_mask
from hftrainer.models.motion.kimodo.network.postprocess import post_process_motion
from hftrainer.models.motion.kimodo.network.sanitize import sanitize_texts
from hftrainer.models.motion.kimodo.network.skeleton import SOMASkeleton30
from hftrainer.models.motion.kimodo.network.tools import to_numpy
...
class Kimodo(nn.Module):
    ...
    def _generate(
        ...
        for i in progress_bar(indices):
            ...
            if (os.environ.get("KIMODO_REPAINT_CONDITION", "0") == "1"
                    and motion_mask is not None and observed_motion is not None):
                next_i = max(i - 1, 0)
                if i > 0:
                    t_next = torch.tensor(
                        [next_i] * cur_mot.size(0), device=self.device)
                    observed_at_t = self.diffusion.q_sample(
                        observed_motion, t_next)
                else:
