...
_NATIVE_PACKAGE = "hftrainer.models.motion.kimodo.network"
TEXT_ENCODER_PRESETS = {
    "dummy": {
        "target": f"{_NATIVE_PACKAGE}.model.DummyTextEncoder",
        "kwargs": {
            "llm_dim": 4096,
        },
    },
    "llm2vec": {
        "target": f"{_NATIVE_PACKAGE}.model.LLM2VecEncoder",
        "kwargs": {
...
def _build_api_text_encoder_conf(text_encoder_url: str) -> dict:
    return {
        "_target_": f"{_NATIVE_PACKAGE}.model.text_encoder_api.TextEncoderAPI",
...
def _rewrite_native_targets(value):
    ...
    if isinstance(value, str) and value.startswith("kimodo."):
        return f"{_NATIVE_PACKAGE}.{value[len('kimodo.'):]}"
    return value
