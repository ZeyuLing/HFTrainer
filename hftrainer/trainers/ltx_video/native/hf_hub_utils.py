...
def push_to_hub(
    weights_path: Path,
    sampled_videos_paths: list[Path] | None,
    ...
    hub, hub_utils = require_huggingface_hub()
    from rich.progress import Progress, SpinnerColumn, TextColumn

    api = hub.HfApi()

    # Save original progress bar state
    original_progress_state = hub_utils.are_progress_bars_disabled()
    hub_utils.disable_progress_bars()  # Disable during our custom progress tracking

    try:
        # Try to create repo if it doesn't exist
        try:
            repo = hub.create_repo(
...
def convert_video_to_gif(video_path: Path, output_path: Path) -> None:
    ...
    try:
        ...
        writer.close()
        reader.close()
    except OptionalLTXDependencyError:
        raise
    except Exception as e:
