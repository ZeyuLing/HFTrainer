...
from hftrainer.models.ltx_video.network.components.diffusion_steps import (
...
from hftrainer.models.ltx_video.network.devices import highest_precision_float
from hftrainer.models.ltx_video.network.model.transformer import X0Model
from hftrainer.models.ltx_video.network.utils import to_denoised, to_velocity
from hftrainer.pipelines.ltx_video.backend.utils.helpers import post_process_latent, timesteps_from_mask
from hftrainer.pipelines.ltx_video.backend.utils.res2s import get_res2s_coefficients
from hftrainer.pipelines.ltx_video.backend.utils.types import DenoisedLatentResult, Denoiser, LatentState
...
def euler_denoising_loop(
    ...
    denoiser:
        ...
        and must return a :class:`~hftrainer.pipelines.ltx_video.backend.utils.types.DenoisedLatentResult`.
    ### Returns
    tuple[LatentState | None, LatentState | None]
...
def euler_ancestral_denoising_loop(
    ...
    :class:`~hftrainer.models.ltx_video.network.components.diffusion_steps.EulerAncestralDiffusionStep`.
    ...
    video_state:
        Current video :class:`~hftrainer.models.ltx_video.network.types.LatentState`, or ``None``.
    audio_state:
        Current audio :class:`~hftrainer.models.ltx_video.network.types.LatentState`, or ``None``.
    stepper:
        :class:`~hftrainer.models.ltx_video.network.components.diffusion_steps.EulerAncestralDiffusionStep`
    ...
    transformer:
        The diffusion model passed to the denoiser at each step.
    denoiser:
        Callable implementing :class:`~hftrainer.pipelines.ltx_video.backend.utils.types.Denoiser`.
    noise_seed:
...
def euler_cfg_pp_denoising_loop(
    ...
    Requires a guided denoiser whose :class:`~hftrainer.pipelines.ltx_video.backend.utils.types.DenoisedLatentResult`
    ...
    audio_state:
        Current audio :class:`~hftrainer.models.ltx_video.network.types.LatentState`, or ``None``.
    stepper:
        :class:`~hftrainer.models.ltx_video.network.components.diffusion_steps.EulerCfgPpDiffusionStep`
        instance carrying ``eta`` and ``s_noise`` parameters.
    transformer:
        The diffusion model passed to the denoiser at each step.
    denoiser:
