...
from hftrainer.models.ltx_video.network.hdr import HDRTransfer
...
from hftrainer.pipelines.ltx_video.backend.utils.media_io.exr import is_exr_dir, load_exr_image_conditioning_hdr
from hftrainer.pipelines.ltx_video.backend.utils.media_io.range_map import normalize_images, to_vae_range
from hftrainer.pipelines.ltx_video.backend.utils.media_io.resize import (
    ResizeMode,
...
def get_videostream_metadata(path: str, fps: float | None = None) -> VideoPixelShape:
    ...
    if is_exr_dir(path):
        ...
        try:
            ...
            raise RuntimeError(
                'EXR metadata requires OpenImageIO. Install '
                'hftrainer[ltx-video-hdr] or use a standard video container.'
            ) from exc
