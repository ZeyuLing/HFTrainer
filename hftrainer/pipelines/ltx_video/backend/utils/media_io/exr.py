...
from hftrainer.pipelines.ltx_video.backend.utils.media_io.range_map import to_vae_range
from hftrainer.pipelines.ltx_video.backend.utils.media_io.resize import ResizeMode, resize_and_center_crop, resize_and_reflect_pad

logger = logging.getLogger(__name__)


def _require_openimageio():
    try:
        import OpenImageIO
    except ImportError as exc:
        raise RuntimeError(
            'EXR/HDR media support requires OpenImageIO. Install '
            'hftrainer[ltx-video-hdr] or use a standard image/video format.'
        ) from exc
    return OpenImageIO
...
def read_exr(path: str | Path) -> torch.Tensor:
    ...
    """
    OpenImageIO = _require_openimageio()
