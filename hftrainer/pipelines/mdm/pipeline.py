...
Uses the hftrainer-native MDM Gaussian-diffusion ancestral sampler
(``hftrainer.models.motion.mdm.network``) to guarantee parity with the released
checkpoint, while exposing the hftrainer-native ``infer_t2m`` task interface.
"""
...
class MDMPipeline(BasePipeline):
    ...
    BUNDLE_CLS = "hftrainer.models.motion.mdm.MDMBundle"

    def __init__(self, bundle, device: Optional[str] = None, **kwargs):
        super().__init__(bundle, **kwargs)
        from hftrainer.models.motion.mdm.network import collate
