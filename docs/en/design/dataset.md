...
Some task stacks may still inherit directly from `mmengine.dataset.BaseDataset` instead of `PipelineDataset` when they need more task-specific sampling or refetch behavior. The same rule still applies: concrete IO and data operations belong in transforms, not in the dataset body.
