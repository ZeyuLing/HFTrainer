import torch
import numpy as np
from tmr_utils.guofeats import joints_to_guofeats
from tmr_utils.model.tmr import get_score_matrix
from tmr_utils.load_tmr_model import load_tmr_model_easy
from tqdm import tqdm
import multiprocessing
from functools import partial
import warnings
warnings.filterwarnings("ignore")
## Borrowed from https://github.com/zju3dv/GVHMR/blob/44fbc1999dd57d2180e882c62c78d356d4c840d0/hmr4d/utils/eval/eval_utils.py

# ===== Utilities =====

FEET2METER = 0.3048

def compute_jpe(S1, S2):
    # (L, J, 3)
    return torch.sqrt(((S1 - S2) ** 2).sum(dim=-1)).mean(dim=-1).numpy() #(L,)
    # return torch.norm(S1 - S2, dim=-1).mean().numpy()

def compute_jpe_wrapper(motion1, motion2, motion_lengths, local=True):
    mpjpe_mean = []
    for idx, (S1, S2, mlen) in enumerate(zip(motion1, motion2, motion_lengths)):
        S1 = S1[:mlen].copy()
        S2 = S2[:mlen].copy()
        if local:
            S1 -= S1[:, [0]]
            S2 -= S2[:, [0]]
        mpjpe = compute_jpe(torch.from_numpy(S1), torch.from_numpy(S2))
        mpjpe_mean.append(mpjpe.mean())
    return np.mean(mpjpe_mean) * FEET2METER * 100 #cm

def compute_perjoint_jpe(S1, S2):
    return torch.sqrt(((S1 - S2) ** 2).sum(dim=-1)).numpy()


def batch_align_by_pelvis(data_list, pelvis_idxs=[1, 2]):
    """
    Assumes data is given as [pred_j3d, target_j3d, pred_verts, target_verts].
    Each data is in shape of (frames, num_points, 3)
    Pelvis is notated as one / two joints indices.
    Align all data to the corresponding pelvis location.
    """

    pred_j3d, target_j3d, pred_verts, target_verts = data_list

    pred_pelvis = pred_j3d[:, pelvis_idxs].mean(dim=1, keepdims=True).clone()
    target_pelvis = target_j3d[:, pelvis_idxs].mean(dim=1, keepdims=True).clone()

    # Align to the pelvis
    pred_j3d = pred_j3d - pred_pelvis
    target_j3d = target_j3d - target_pelvis
    pred_verts = pred_verts - pred_pelvis
    target_verts = target_verts - target_pelvis

    return (pred_j3d, target_j3d, pred_verts, target_verts)


def batch_compute_similarity_transform_torch(S1, S2):
    """
    Computes a similarity transform (sR, t) that takes
    a set of 3D points S1 (3 x N) closest to a set of 3D points S2,
    where R is an 3x3 rotation matrix, t 3x1 translation, s scale.
    i.e. solves the orthogonal Procrutes problem.
    """
    transposed = False
    if S1.shape[0] != 3 and S1.shape[0] != 2:
        S1 = S1.permute(0, 2, 1)
        S2 = S2.permute(0, 2, 1)
        transposed = True
    assert S2.shape[1] == S1.shape[1]

    # 1. Remove mean.
    mu1 = S1.mean(axis=-1, keepdims=True)
    mu2 = S2.mean(axis=-1, keepdims=True)

    X1 = S1 - mu1
    X2 = S2 - mu2

    # 2. Compute variance of X1 used for scale.
    var1 = torch.sum(X1**2, dim=1).sum(dim=1)

    # 3. The outer product of X1 and X2.
    K = X1.bmm(X2.permute(0, 2, 1))

    # 4. Solution that Maximizes trace(R'K) is R=U*V', where U, V are
    # singular vectors of K.
    U, s, V = torch.svd(K)

    # Construct Z that fixes the orientation of R to get det(R)=1.
    Z = torch.eye(U.shape[1], device=S1.device).unsqueeze(0)
    Z = Z.repeat(U.shape[0], 1, 1)
    Z[:, -1, -1] *= torch.sign(torch.det(U.bmm(V.permute(0, 2, 1))))

    # Construct R.
    R = V.bmm(Z.bmm(U.permute(0, 2, 1)))

    # 5. Recover scale.
    scale = torch.cat([torch.trace(x).unsqueeze(0) for x in R.bmm(K)]) / var1

    # 6. Recover translation.
    t = mu2 - (scale.unsqueeze(-1).unsqueeze(-1) * (R.bmm(mu1)))

    # 7. Error:
    S1_hat = scale.unsqueeze(-1).unsqueeze(-1) * R.bmm(S1) + t

    if transposed:
        S1_hat = S1_hat.permute(0, 2, 1)

    return S1_hat


def compute_error_accel(joints_gt, joints_pred, valid_mask=None, fps=None):
    """
    Use [i-1, i, i+1] to compute acc at frame_i. The acceleration error:
        1/(n-2) \sum_{i=1}^{n-1} X_{i-1} - 2X_i + X_{i+1}
    Note that for each frame that is not visible, three entries(-1, 0, +1) in the
    acceleration error will be zero'd out.
    Args:
        joints_gt : (F, J, 3)
        joints_pred : (F, J, 3)
        valid_mask : (F)
    Returns:
        error_accel (F-2) when valid_mask is None, else (F'), F' <= F-2
    """
    # (F, J, 3) -> (F-2) per-joint
    accel_gt = joints_gt[:-2] - 2 * joints_gt[1:-1] + joints_gt[2:]
    accel_pred = joints_pred[:-2] - 2 * joints_pred[1:-1] + joints_pred[2:]
    normed = np.linalg.norm(accel_pred - accel_gt, axis=-1).mean(axis=-1)
    if fps is not None:
        normed = normed * fps**2

    if valid_mask is None:
        new_vis = np.ones(len(normed), dtype=bool)
    else:
        invis = np.logical_not(valid_mask)
        invis1 = np.roll(invis, -1)
        invis2 = np.roll(invis, -2)
        new_invis = np.logical_or(invis, np.logical_or(invis1, invis2))[:-2]
        new_vis = np.logical_not(new_invis)
        if new_vis.sum() == 0:
            print("Warning!!! no valid acceleration error to compute.")

    return normed[new_vis]

def compute_error_accel_wrapper(motion1, motion2, motion_lengths, fps=20):
    error_accel_mean = []
    for idx, (S1, S2, mlen) in enumerate(zip(motion1, motion2, motion_lengths)):
        S1 = S1[:mlen].copy()
        S2 = S2[:mlen].copy()
        error_accel = compute_error_accel(torch.from_numpy(S1), torch.from_numpy(S2), fps=fps)
        error_accel_mean.append(error_accel.mean())
    return np.mean(error_accel_mean) * FEET2METER #m


def compute_rte_deprecated(target_trans, pred_trans, align=True):
    # Compute the global alignment
    if align:
        _, rot, trans = align_pcl(target_trans[None, :], pred_trans[None, :], fixed_scale=True)
        pred_trans_hat = (torch.einsum("tij,tnj->tni", rot, pred_trans[None, :]) + trans[None, :])[0]
    else:
        pred_trans_hat = pred_trans

    # Compute the entire displacement of ground truth trajectory
    disps, disp = [], 0
    for p1, p2 in zip(target_trans, target_trans[1:]):
        delta = (p2 - p1).norm(2, dim=-1)
        disp += delta
        disps.append(disp)

    # Compute absolute root-translation-error (RTE)
    rte = torch.norm(target_trans - pred_trans_hat, 2, dim=-1)

    # Normalize it to the displacement, TODO why normalize, lead to bad results. attach more importance to stationarys
    return (rte / disp).numpy()

def compute_rte(target_trans, pred_trans, align=True):
    # Compute the global alignment
    if align:
        _, rot, trans = align_pcl(target_trans[None, :], pred_trans[None, :], fixed_scale=True)
        pred_trans_hat = (torch.einsum("tij,tnj->tni", rot, pred_trans[None, :]) + trans[None, :])[0]
    else:
        pred_trans_hat = pred_trans

    # Compute absolute root-translation-error (RTE)
    rte = torch.norm(target_trans - pred_trans_hat, 2, dim=-1)

    # Normalize it to the displacement
    return rte.numpy()

def compute_rte_wrapper(motion1, motion2, motion_lengths, align=True):
    rte_mean = []
    for idx, (S1, S2, mlen) in enumerate(zip(motion1, motion2, motion_lengths)):
        S1 = S1[:mlen].copy()
        S1 = S1[:, 0]
        S2 = S2[:mlen].copy()
        S2 = S2[:, 0]
        rte = compute_rte(torch.from_numpy(S1).float(), torch.from_numpy(S2).float(), align=align)
        rte_mean.append(rte.mean())
    return np.mean(rte_mean) #relative

def compute_jitter(joints, fps=20):
    """compute jitter of the motion
    Args:
        joints (N, J, 3).
        fps (float).
    Returns:
        jitter (N-3).
    """
    pred_jitter = torch.norm(
        (joints[3:] - 3 * joints[2:-1] + 3 * joints[1:-2] - joints[:-3]) * (fps**3),
        dim=-1,
    ).mean(dim=-1)

    return pred_jitter.cpu().numpy() / 10.0 * FEET2METER

def compute_jitter_wrapper(joints, motion_lengths, fps=20):
    """compute jitter of the motion
    Args:
        joints (B, N, J, 3).
        fps (float).
    Returns:
        jitter (Num of frames).
    """
    assert len(joints) == len(motion_lengths)
    B = len(joints)
    re = []
    for i in range(B):
        re.append(compute_jitter(torch.from_numpy(joints[i][:motion_lengths[i]]), fps))
    return np.concatenate(re, axis=0)

def compute_jittering(output_seqs):
    ## from AMDM
    a_seqs = np.diff(output_seqs, n=2, axis=-1)
    a_norm = np.linalg.norm(a_seqs,axis=-1)
    return a_norm

def compute_foot_sliding_smpl(target_verts, pred_verts, thr=1e-2):
    """compute foot sliding error
    The foot ground contact label is computed by the threshold of 1 cm/frame
    Args:
        target_verts (N, 6890, 3).
        pred_verts (N, 6890, 3).
    Returns:
        error (N frames in contact).
    """
    assert target_verts.shape == pred_verts.shape
    assert target_verts.shape[-2] == 6890

    # Foot vertices idxs
    foot_idxs = [3216, 3387, 6617, 6787]

    # Compute contact label
    foot_loc = target_verts[:, foot_idxs]
    foot_disp = (foot_loc[1:] - foot_loc[:-1]).norm(2, dim=-1)
    contact = foot_disp[:] < thr

    pred_feet_loc = pred_verts[:, foot_idxs]
    pred_disp = (pred_feet_loc[1:] - pred_feet_loc[:-1]).norm(2, dim=-1)

    error = pred_disp[contact]

    return error.cpu().numpy()


def convert_joints22_to_24(joints22, ratio2220=0.3438, ratio2321=0.3345):
    joints24 = torch.zeros(*joints22.shape[:-2], 24, 3).to(joints22.device)
    joints24[..., :22, :] = joints22
    joints24[..., 22, :] = joints22[..., 20, :] + ratio2220 * (joints22[..., 20, :] - joints22[..., 18, :])
    joints24[..., 23, :] = joints22[..., 21, :] + ratio2321 * (joints22[..., 21, :] - joints22[..., 19, :])
    return joints24


def align_pcl(Y, X, weight=None, fixed_scale=False):
    """align similarity transform to align X with Y using umeyama method
    X' = s * R * X + t is aligned with Y
    :param Y (*, N, 3) first trajectory
    :param X (*, N, 3) second trajectory
    :param weight (*, N, 1) optional weight of valid correspondences
    :returns s (*, 1), R (*, 3, 3), t (*, 3)
    """
    *dims, N, _ = Y.shape
    N = torch.ones(*dims, 1, 1) * N

    if weight is not None:
        Y = Y * weight
        X = X * weight
        N = weight.sum(dim=-2, keepdim=True)  # (*, 1, 1)

    # subtract mean
    my = Y.sum(dim=-2) / N[..., 0]  # (*, 3)
    mx = X.sum(dim=-2) / N[..., 0]
    y0 = Y - my[..., None, :]  # (*, N, 3)
    x0 = X - mx[..., None, :]

    if weight is not None:
        y0 = y0 * weight
        x0 = x0 * weight

    # correlation
    C = torch.matmul(y0.transpose(-1, -2), x0) / N  # (*, 3, 3)
    U, D, Vh = torch.linalg.svd(C)  # (*, 3, 3), (*, 3), (*, 3, 3)

    S = torch.eye(3).reshape(*(1,) * (len(dims)), 3, 3).repeat(*dims, 1, 1)
    neg = torch.det(U) * torch.det(Vh.transpose(-1, -2)) < 0
    S[neg, 2, 2] = -1

    R = torch.matmul(U, torch.matmul(S, Vh))  # (*, 3, 3)

    D = torch.diag_embed(D)  # (*, 3, 3)
    if fixed_scale:
        s = torch.ones(*dims, 1, device=Y.device, dtype=torch.float32)
    else:
        var = torch.sum(torch.square(x0), dim=(-1, -2), keepdim=True) / N  # (*, 1, 1)
        s = torch.diagonal(torch.matmul(D, S), dim1=-2, dim2=-1).sum(dim=-1, keepdim=True) / var[..., 0]  # (*, 1)

    t = my - s * torch.matmul(R, mx[..., None])[..., 0]  # (*, 3)

    return s, R, t


def global_align_joints(gt_joints, pred_joints):
    """
    :param gt_joints (T, J, 3)
    :param pred_joints (T, J, 3)
    """
    s_glob, R_glob, t_glob = align_pcl(gt_joints.reshape(-1, 3), pred_joints.reshape(-1, 3))
    pred_glob = s_glob * torch.einsum("ij,tnj->tni", R_glob, pred_joints) + t_glob[None, None]
    return pred_glob


def first_align_joints(gt_joints, pred_joints):
    """
    align the first two frames
    :param gt_joints (T, J, 3)
    :param pred_joints (T, J, 3)
    """
    # (1, 1), (1, 3, 3), (1, 3)
    s_first, R_first, t_first = align_pcl(gt_joints[:2].reshape(1, -1, 3), pred_joints[:2].reshape(1, -1, 3))
    pred_first = s_first * torch.einsum("tij,tnj->tni", R_first, pred_joints) + t_first[:, None]
    return pred_first


def rearrange_by_mask(x, mask):
    """
    x (L, *)
    mask (M,), M >= L
    """
    M = mask.size(0)
    L = x.size(0)
    if M == L:
        return x
    assert M > L
    assert mask.sum() == L
    x_rearranged = torch.zeros((M, *x.size()[1:]), dtype=x.dtype, device=x.device)
    x_rearranged[mask] = x
    return x_rearranged


def as_np_array(d):
    if isinstance(d, torch.Tensor):
        return d.cpu().numpy()
    elif isinstance(d, np.ndarray):
        return d
    else:
        return np.array(d)
    
def compute_label_metrics(preds, gts, lengths):
    from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score
    from tqdm import tqdm
    acc = []
    pre = []
    rec = []
    f1 = []
    for i in tqdm(range(len(lengths))):
        temp_gt = np.array(gts[i][:lengths[i]], dtype=bool)
        temp_pred = np.array(preds[i][:lengths[i]], dtype=bool)
        _acc = accuracy_score(temp_gt, temp_pred)
        _precision = precision_score(temp_gt, temp_pred, zero_division=np.nan)
        _recall = recall_score(temp_gt, temp_pred, zero_division=np.nan)
        _f1_score = f1_score(temp_gt, temp_pred, zero_division=np.nan)
        acc.append(_acc)
        pre.append(_precision)
        rec.append(_recall)
        f1.append(_f1_score)

    return np.nanmean(acc), np.nanmean(pre), np.nanmean(rec), np.nanmean(f1)

def compute_skating_ratio_zup(joints, offseth=None):
    # from rohm
    thresh_height = 0.10
    thresh_vel = 0.10
    fps = 20
    foot_joint_index_list = [7, 10, 8, 11]  # contact lbl dim order: 7, 10, 8, 11, left ankle, toe, right angle, toe
    joints_foot = joints[:, foot_joint_index_list, :] * FEET2METER  # [clip_len,  4, 3]
    if offseth == None:
        offseth = np.mean(joints_foot[:10, :, 2])
    else:
        offseth *= FEET2METER
    joints_feet_horizon_vel = np.linalg.norm(joints_foot[1:, :, [0, 1]] - joints_foot[:-1, :, [0, 1]], axis=-1) * fps  # [lip_len, 4]
    joints_feet_height = joints_foot[0:-1, :, 2]  # [clip_len, 4]
    joints_feet_height = joints_feet_height - offseth
    skating_left = (joints_feet_horizon_vel[:, 0] > thresh_vel) * (joints_feet_horizon_vel[:, 1] > thresh_vel) * \
                      (joints_feet_height[:, 0] < (thresh_height + 0.05)) * (joints_feet_height[:, 1] < thresh_height)
    skating_right = (joints_feet_horizon_vel[:, 2] > thresh_vel) * (joints_feet_horizon_vel[:, 3] > thresh_vel) * \
                       (joints_feet_height[:, 2] < (thresh_height + 0.05)) * (joints_feet_height[:, 3] < thresh_height)
    skating = skating_left * skating_right
    skating_ratio = np.mean(skating)
    return skating_ratio

def compute_skating_ratio_wrapper(motions, motion_lengths, return_list=False):
    sr_mean = []
    for idx, (motion, mlen) in enumerate(zip(motions, motion_lengths)):
        motion = motion[:mlen]
        offseth = motion.min(axis=0).min(axis=0)[2]
        sr = compute_skating_ratio_zup(motion, offseth)
        # print(idx, sliding)
        sr_mean.append(sr)
    if return_list:
        return np.mean(sr_mean), sr_mean
    return np.mean(sr_mean)

def compute_ground_penetration_zup(joints, offseth=None):
    # from rohm
    foot_joint_index_list = [10, 11] 
    joints_foot = joints[:, foot_joint_index_list, :] * FEET2METER  # [clip_len,  2, 3]
    if offseth == None:
        offseth = np.mean(joints_foot[:10, :, 2])
    else:
        offseth *= FEET2METER
    pene_dist = joints_foot[..., 2] - offseth  # [clip_len, 2]
    pene_freq = pene_dist < -0.05  # [clip_len]
    pene_freq = pene_freq.mean()
    pene_dist[pene_dist >= 0] = 0
    pene_dist = pene_dist.mean()
    return pene_freq, pene_dist*1000

def compute_gpenetration_wrapper(motions, motion_lengths, offseth=None):
    pf_mean = []
    pd_mean = []
    for idx, (motion, mlen) in enumerate(zip(motions, motion_lengths)):
        motion = motion[:mlen]
        pf, pd = compute_ground_penetration_zup(motion, offseth)
        pf_mean.append(pf)
        pd_mean.append(pd)
    return np.mean(pf_mean), np.mean(pd_mean)

def compute_foot_sliding(foot_data, traj_qpos, offseth=None, upaxis=1, H=0.05):
    # from momask, genmostyle, kinpoly, omomo
    plane_axis = [0, 1, 2]
    plane_axis.pop(upaxis) 
    foot = np.array(foot_data).copy() * FEET2METER
    if offseth == None:
        offseth = np.mean(foot[:10, upaxis])
    else:
        offseth *= FEET2METER

    foot[:, upaxis] -= offseth  # Grounding it
    foot_disp = np.linalg.norm(foot[1:, plane_axis] - foot[:-1, plane_axis], axis=1)
    traj_qpos[:, upaxis] -= offseth
    seq_len = len(traj_qpos)
    H = H
    y_threshold = 0.65  # yup system
    y = traj_qpos[1:, upaxis]

    foot_avg = (foot[:-1, upaxis] + foot[1:, upaxis]) / 2
    subset = np.logical_and(foot_avg < H, y > y_threshold)

    sliding_stats = np.abs(foot_disp * (2 - 2 ** (foot_avg / H)))[subset]
    sliding = np.sum(sliding_stats) / seq_len * 1000
    return sliding, sliding_stats

def compute_foot_sliding_wrapper(motions, motion_lengths, upaxis=1, ankle_h=0.05):
    traj_idx = 0
    feet_idxs = [7, 8]
    sliding_mean = []
    for idx, (motion, mlen) in enumerate(zip(motions, motion_lengths)):
        motion = motion[:mlen]
        traj_qpos = motion[:, traj_idx]
        offseth = motion.min(axis=0).min(axis=0)[upaxis]
        for foot_idx in feet_idxs:
            foot_data = motion[:, foot_idx]
            sliding, _ = compute_foot_sliding(foot_data, traj_qpos, offseth, upaxis=upaxis, H=ankle_h)
            sliding_mean.append(sliding)
    return np.mean(sliding_mean)


def compute_foot_sliding_torch(foot_data, traj_qpos, offseth=None, upaxis=1, H=0.05):
    # from momask, genmostyle, kinpoly, omomo
    plane_axis = [0, 1, 2]
    plane_axis.pop(upaxis) 
    foot = foot_data.clone() * FEET2METER
    if offseth == None:
        offseth = torch.mean(foot[:10, upaxis])
    else:
        offseth = offseth * FEET2METER
    _foot = foot.clone()
    _traj_qpos = traj_qpos.clone()
    _foot[:, upaxis] = foot[:, upaxis] - offseth  # Grounding it
    _traj_qpos[:, upaxis] = traj_qpos[:, upaxis] - offseth
    foot = _foot
    traj_qpos = _traj_qpos
    foot_disp = torch.linalg.norm(foot[1:, plane_axis] - foot[:-1, plane_axis], dim=1)
    seq_len = len(traj_qpos)
    H = H 
    y_threshold = 0.65  # yup system
    y = traj_qpos[1:, upaxis]

    foot_avg = (foot[:-1, upaxis] + foot[1:, upaxis]) / 2
    subset = torch.logical_and(foot_avg < H, y > y_threshold)

    sliding_stats = torch.abs(foot_disp * (2 - 2 ** (foot_avg.detach() / H)))[subset]
    sliding = torch.sum(sliding_stats)
    return sliding, sliding_stats

def compute_foot_sliding_wrapper_torch(motions, motion_lengths, upaxis=1, ankle_h=0.05):
    traj_idx = 0
    feet_idxs = [7, 8]
    sliding_mean = []
    for idx, (motion, mlen) in enumerate(zip(motions, motion_lengths)):
        motion = motion[:mlen]
        traj_qpos = motion[:, traj_idx]
        offseth = torch.min(motion[..., upaxis]).detach()
        for foot_idx in feet_idxs:
            foot_data = motion[:, foot_idx]
            sliding, _ = compute_foot_sliding_torch(foot_data, traj_qpos, offseth, upaxis=upaxis, H=ankle_h)
            sliding_mean.append(sliding)
    return sliding_mean # List

def T(x):
    if isinstance(x, torch.Tensor):
        return x.permute(*torch.arange(x.ndim - 1, -1, -1))
    else:
        return x.transpose(*np.arange(x.ndim - 1, -1, -1))



def tmr_gfeats(data, y_is_z_axis):
    motions, gt_motions, lengths = data
    assert motions.shape[-1] == 3
    assert motions.shape[-2] == 24 or motions.shape[-2] == 22
    
    assert gt_motions.shape[-1] == 3
    assert gt_motions.shape[-2] == 24 or gt_motions.shape[-2] == 22

    if motions.shape[-2] == 24:
        motions = motions[..., :22, :]
    if gt_motions.shape[-2] == 24:
        gt_motions = gt_motions[..., :22, :]
    
    if not y_is_z_axis:
        x, y, z = T(motions)
        motions = T(np.stack((x, z, -y), axis=0))
        x, y, z = T(gt_motions)
        gt_motions = T(np.stack((x, z, -y), axis=0))

    gfeats = joints_to_guofeats(motions[:lengths])
    gt_gfeats = joints_to_guofeats(gt_motions[:lengths])

    return gfeats, gt_gfeats

from PIL import Image
def tmr_m2m_score_wrapper(motions, gt_motions, lengths, y_is_z_axis, device='cpu', bs=1):
    tmr_forward = load_tmr_model_easy(device)
    m2m_score_mean = []
    gfeats_lst = []
    gt_gfeats_lst = []
    m2m_top_1_lst = []
    m2m_top_3_lst = []
    my_function = partial(tmr_gfeats, y_is_z_axis=y_is_z_axis)
    with multiprocessing.Pool(processes=32) as pool:
        # Map the function to a list of inputs and collect the results
        inputs = zip(motions, gt_motions, lengths)
        results = pool.map(my_function, inputs)
    gfeats_lst = [ret[0] for ret in results]
    gt_gfeats_lst = [ret[1] for ret in results]

    motion_latent = tmr_forward(gfeats_lst)
    gt_motion_latent = tmr_forward(gt_gfeats_lst)
    sim_m2m = get_score_matrix(motion_latent, gt_motion_latent, device)
    # Image.fromarray((sim_m2m.cpu().numpy()*255).astype(np.uint8)).save(f'sim_map.png')
    for sim_idx in range(len(motion_latent)):
        m2m_score_mean.append(sim_m2m[sim_idx, sim_idx].cpu().numpy())
        asort = np.argsort(sim_m2m[sim_idx].cpu().numpy())[::-1]
        m2m_top_1_lst.append(1 * (sim_idx in asort[:1]))
        m2m_top_3_lst.append(1 * (sim_idx in asort[:3]))
    return np.mean(m2m_score_mean), np.mean(m2m_top_1_lst), np.mean(m2m_top_3_lst)
