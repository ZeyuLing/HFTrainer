...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        self.samples, self.bs, self.njoints, self.nfeats, self.nframes = self.motions['motion'].shape
        self.opt_cache = {}
        ...
        if self.nfeats == 3:
            ...
            motion_tensor, opt_dict = self.j2s.joint2smpl(self.motions['motion'][self.sample_idx][self.rep_idx].transpose(2, 0, 1))  # [nframes, njoints, 3]
        ...
        self.real_num_frames = self.motions['lengths'][self.sample_idx][self.rep_idx]
        print(self.motions['lengths'], self.motions['lengths'].shape)
        print(self.real_num_frames)
