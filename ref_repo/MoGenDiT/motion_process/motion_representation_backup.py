...
class OccamMotionRep:
    ...
    def geometric_degradation_batch(
        ...
        degradation_scales = torch.rand((b, 2, 1, 1))
        degradation_scales = degradation_scales.reshape(b * 2, 1, 1)
        ...
        rotation_degration_local[..., 1] = rotation_degration_local[..., 1].clip(
            min=-np.pi / 2, max=np.pi / 2
        )

        # 0-10% 的概率出现local关节跳变
        rotation_degration_mask_local = torch.bernoulli(
            torch.ones(b, 1, 1, 1).repeat(1, seq_len, self.n_joint, 1)
            * torch.rand(b, 1, 1, 1)
            * 0.1
        )
        rotation_degration_local *= rotation_degration_mask_local

        rotation_degration_matrix_local = euler_angles_to_matrix(
            rotation_degration_local.reshape(-1, 3).to(motion.device), convention="XYZ"
        ).reshape(b, -1, self.n_joint, 3, 3)

        # global空间joint跳变
        rotation_degration_global = (
            ((torch.rand(b, seq_len, self.n_joint, 3)) - 0.5)
            * 2
            * (np.pi / 180)
            * 180.0
        ...
        # 0-10% 的概率出现关节跳变
        rotation_degration_mask_global = torch.bernoulli(
            torch.ones(b, 1, 1, 1).repeat(1, seq_len, self.n_joint, 1)
            * torch.rand(b, 1, 1, 1)
            * 0.1
        )
        rotation_degration_global *= rotation_degration_mask_global

        rotation_degration_matrix_global = euler_angles_to_matrix(
            rotation_degration_global.reshape(-1, 3).to(motion.device), convention="XYZ"
        ).reshape(b, -1, self.n_joint, 3, 3)
        ...
        gaussian_delta_y_drift = (
            torch.randn(b, seq_len, 1) * 0.05 * degradation_scale_trans
        )
        if length is not None:
        ...
        if self.global_pose:
            # 先使用local pose
            pose = self.body_model.inverse_kinematics(R=pose)
        pose = pose.matmul(rotation_degration_matrix_local)
        pose = self.body_model.forward_kinematics(R=pose)
        pose = pose.matmul(rotation_degration_matrix_global)
        joint = self.body_model.joint_fk_global(
            R=pose.reshape(b, -1, self.n_joint, 3, 3),
            skeleton_offsets=init_skeleton_offsets.reshape(b, -1, self.n_joint, 3),
        ).reshape(b, -1, self.n_joint * 3)
