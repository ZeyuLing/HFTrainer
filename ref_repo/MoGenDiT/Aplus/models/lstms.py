...
class EasyLSTM(BaseModel):
    def __init__(self, n_input, n_output, n_hidden, n_lstm_layer, bidirectional=False, dropout=0.2,
                 output_type='seq', use_cell_state=True, act_func='relu', layer_norm=False):
        """
        ...
        self.activation_func = activation_layer(act_name=act_func)
        ...
        if layer_norm:
            self.fc_2 = nn.Sequential(nn.LayerNorm(lstm_out_dim), nn.Linear(lstm_out_dim, n_output, bias=False))
        else:
...
class EasyRNN(BaseModel):
    def __init__(self, n_input, n_output, n_hidden, n_rnn_layer, bidirectional=False, dropout=0.2,
                 output_type='seq', act_func='relu'):
        """
