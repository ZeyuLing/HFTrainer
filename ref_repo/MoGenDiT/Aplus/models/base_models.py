...
class BaseModel(nn.Module):
    ...
    def get_avg_parameter_norm(self):
        """
        计算模型所有可学习参数的平均L2范数
        L2范数定义: 对于参数张量x, 范数为sqrt(sum(x_i²))
        """
        total_norm = 0.0  # 所有参数范数的总和
        param_count = 0  # 参数的数量（按张量计数，而非元素数量）

        for param in self.parameters():
            if param.requires_grad:  # 只考虑可学习的参数
                # 计算当前参数张量的L2范数
                param_norm = param.norm(p=2).item()  # p=2表示L2范数
                total_norm += param_norm
                param_count += 1

        if param_count == 0:
            return 0.0  # 没有可学习参数时返回0
