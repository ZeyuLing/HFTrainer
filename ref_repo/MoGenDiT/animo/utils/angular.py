...
def normalize_angle(q):
    ...
    q = copy.deepcopy(q.reshape(-1, 3))
    q_xz = q[:, [0, 2]]
    q_xz = q_xz % (2 * np.pi)
    q_xz[q_xz >= np.pi] -= 2 * np.pi

    q_y = q[:, [1]]
    q_y = q_y % np.pi
    q_y[q_y >= np.pi / 2] -= np.pi
...
def angle_between(
    rot1: torch.Tensor, rot2: torch.Tensor, rep=RotationRepresentation.ROTATION_MATRIX
):
    r"""
...
def rotation_matrix_to_axis_angle(r: torch.Tensor):
    ...
    result = [
        cv2.Rodrigues(_)[0] for _ in r.clone().detach().cpu().view(-1, 3, 3).numpy()
    ]
...
def rotation_matrix_to_r6d(r: torch.Tensor):
    ...
    """
    return r.reshape(-1, 3, 3)[:, :, :2].transpose(1, 2).reshape(-1, 6)
...
def imu_rotation_matrix_to_r6d(r: torch.Tensor):
    ...
    imu_num = r.size(1)
    return (
        r.view(batch_size, imu_num, 3, 3)[:, :, :, :2]
        .transpose(2, 3)
        .clone()
        .view(batch_size, imu_num, 6)
    )
...
def quaternion_to_axis_angle(q: torch.Tensor):
    ...
    **Warning**: The returned axis angles may have a rotation larger than 180 degrees (in 180 ~ 360).
    ...
    a = (q[:, 1:] / theta_half.sin().view(-1, 1) * 2 * theta_half.view(-1, 1)).view(
        -1, 3
    )
    a[torch.isnan(a)] = 0
    return a
...
def rotation_matrix_to_euler_angle_torch(r: torch.Tensor, seq="XYZ") -> torch.Tensor:
    #  conda install libpython m2w64-toolchain -c msys2
    #  git clone --filter=blob:none --quiet https://github.com/facebookresearch/pytorch3d.git
    from pytorch3d.transforms import euler_angles_to_matrix, matrix_to_euler_angles

    r"""
    将旋转矩阵转换为欧拉角（PyTorch批量实现）

    :param r: 可重塑为[batch_size, 3, 3]的旋转矩阵张量
    :param seq: 欧拉角旋转顺序，如'XYZ'表示 intrinsic 旋转
    :return: 形状为[batch_size, 3]的欧拉角张量（弧度）
    """
    r"""
        将旋转矩阵转换为欧拉角（使用PyTorch3D实现）
    ...
    # 使用PyTorch3D的矩阵转欧拉角函数
    euler = matrix_to_euler_angles(r, seq=seq)

    return euler


def euler_angle_to_rotation_matrix_torch(q: torch.Tensor, seq="XYZ") -> torch.Tensor:
    from pytorch3d.transforms import euler_angles_to_matrix, matrix_to_euler_angles

    r"""
    将欧拉角转换为旋转矩阵（使用PyTorch3D实现）

    :param q: 可重塑为[batch_size, 3]的欧拉角张量（弧度）
    :param seq: 欧拉角旋转顺序，如'XYZ'表示 intrinsic 旋转
    ...
    # 使用PyTorch3D的欧拉角转矩阵函数
    rot_matrix = euler_angles_to_matrix(q, seq=seq)
