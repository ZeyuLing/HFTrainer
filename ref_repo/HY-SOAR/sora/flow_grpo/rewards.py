...
def unifiedreward_score_sglang(device):
    ...
    async def evaluate_image(prompt, image):
        question = (
            "<image>\nYou are given a text caption and a generated image "
            "based on that caption. Your task is to evaluate this image "
...
def multi_score(device, score_dict):
    ...
    score_fns = {}
    for score_name, _weight in score_dict.items():
    ...
    # only_strict is only for geneval. During training, only the strict reward
    # is needed, and non-strict rewards don't need to be computed,
    # reducing reward calculation time.
