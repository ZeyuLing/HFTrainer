...
def load_geneval(device):
    ...
    def load_models():
        ...
        ckpt_path = os.path.join(CKPT_PATH, f"{object_detector_name}.pth")
    ...
    def color_classification(image, bboxes, classname):
        ...
        with torch.no_grad():
            ...
            return [colors_list[index.item()] for index in pred.argmax(1)]
                ...
                if "color" in req:
                    ...
                    if colors.count(req["color"]) != req["count"]:
                        ...
                        reason.append(
                            f"expected {req['color']} {classname}>={req['count']}, found "
                            + f"{colors.count(req['color'])} {req['color']}; and "
                            + ", ".join(f"{colors.count(c)} {c}" for c in colors_list if c in colors)
                        )
                if "position" in req and matched:
                    # Relative position check
                    expected_rel, target_group = req["position"]
