...
def _load_adaptive_mask_for_motion(
    ...
    **No dilation is applied.** The MoGenDIT adaptive mask flags exactly the
    per-frame per-joint defect regions; expanding it kinematically/temporally
    was found to hurt results (over-masks clean neighbors, amplifies LQ jitter
    contamination in un-masked regions). Callers that want dilation can do
    it themselves on the returned mask.
