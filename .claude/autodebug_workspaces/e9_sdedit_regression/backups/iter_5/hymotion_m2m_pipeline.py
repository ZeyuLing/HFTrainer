...
class HyMotionM2MPipeline:
    ...
    def _inference(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'text_vec_raw' in batch:
            ...
            # Unconditioned inference: broadcast the learned null embedding
            # across all pad_len tokens (training convention for null samples).
            vtxt_input = self.bundle.null_vtxt_feat.to(dtype=model_dtype).expand(B, 1, -1)
            ctxt_input = self.bundle.null_ctxt_input.to(dtype=model_dtype).expand(B, pad_len, -1).contiguous()
            ctxt_length = torch.zeros(B, dtype=torch.long, device=device)
            ctxt_mask_temporal = _length_to_mask(ctxt_length, pad_len)
        ...
        if do_cfg:
            ...
            null_ctxt = self.bundle.null_ctxt_input.to(dtype=model_dtype).expand(B, pad_len, -1).contiguous()
